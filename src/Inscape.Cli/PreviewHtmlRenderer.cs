...
namespace Inscape.Cli {
    ...
    public static class PreviewHtmlRenderer {
        ...
        static string RenderSerializedOutput(object output, JsonSerializerOptions jsonOptions) {
            ...
            html.AppendLine("    body { margin: 0; min-height: 100vh; background: #f7f5ef; color: #211d18; }");
            html.AppendLine("    main { min-height: 100vh; display: flex; align-items: center; justify-content: center; padding: 22px; }");
            ...
            html.AppendLine("    .status { color: #7b7468; font-size: 13px; }");
            html.AppendLine("    button { cursor: pointer; border: 1px solid #d7d1c4; background: #fbfaf6; color: inherit; padding: 9px 13px; border-radius: 999px; font-size: 14px; }");
            html.AppendLine("    button:hover { background: #ffffff; border-color: #bdb4a3; }");
            html.AppendLine("    button:disabled { opacity: 0.45; cursor: default; }");
            html.AppendLine("    .story-card { background: #fffdf9; border: 1px solid #e3ddd0; border-radius: 24px; overflow: hidden; }");
            html.AppendLine("    .story-header { padding: 18px 22px 0; display: flex; align-items: center; gap: 10px; flex-wrap: wrap; }");
            html.AppendLine("    .node-title { font-size: 12px; color: #8d846f; letter-spacing: 0.08em; text-transform: uppercase; }");
            ...
            html.AppendLine("    .meta-pill { display: inline-flex; align-items: center; border-radius: 999px; padding: 4px 10px; background: #f1ece2; color: #706754; font-size: 12px; font-family: \"Cascadia Mono\", Consolas, monospace; }");
            html.AppendLine("    .story-panel { padding: 30px 26px 12px; min-height: 320px; }");
            ...
            html.AppendLine("    .choices { padding: 6px 26px 24px; }");
            html.AppendLine("    .choice-prompt { margin: 0 0 12px; color: #807663; font-size: 14px; }");
            html.AppendLine("    .choice { display: block; width: 100%; text-align: left; margin: 0 0 10px; padding: 16px 18px; border-radius: 16px; font-size: 18px; background: #fffdf9; }");
            html.AppendLine("    .choice:last-child { margin-bottom: 0; }");
            html.AppendLine("    .continue-hint { padding: 0 26px 22px; color: #8d8068; font-size: 13px; }");
            html.AppendLine("    .ending { padding: 0 26px 22px; color: #8d8068; font-size: 13px; }");
            html.AppendLine("    .diagnostics { margin-top: 16px; display: flex; flex-direction: column; gap: 8px; }");
            html.AppendLine("    .diagnostic { border-left: 4px solid #b45635; background: #fff7f2; padding: 10px 12px; border-radius: 10px; color: #7f2f18; font-size: 13px; }");
            ...
            html.AppendLine("    if (vscode) { window.addEventListener('message', event => { const message = event.data || {}; if (message.type === 'refresh') { persistState(); renderDiagnostics(); render(); } }); }");
