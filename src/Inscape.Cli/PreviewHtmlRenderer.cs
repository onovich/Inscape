...
namespace Inscape.Cli {
    ...
    public static class PreviewHtmlRenderer {
        ...
        static string RenderSerializedOutput(object output, JsonSerializerOptions jsonOptions) {
            ...
            html.AppendLine("    function renderSourceButton(source) { const payload = sourcePayload(source); if (!payload) return undefined; const button = document.createElement('button'); button.className = 'source-button'; button.textContent = '源码'; button.title = `${payload.sourcePath}:${payload.line + 1}`; button.onclick = () => openSource(payload); return button; }");
            html.AppendLine("    function renderNodes() { const el = document.getElementById('nodes'); clear(el); graph.nodes.forEach(node => { const row = document.createElement('div'); row.className = 'node-row'; const button = document.createElement('button'); button.className = 'node-button'; button.textContent = node.name; button.onclick = () => go(node.name); row.appendChild(button); const sourceButton = renderSourceButton(node.source); if (sourceButton) row.appendChild(sourceButton); el.appendChild(row); }); }");
            ...
            html.AppendLine("    function renderAnchor(parent, value, source) { if (value) { const anchor = document.createElement('div'); anchor.className = 'meta'; anchor.textContent = `#${value}`; parent.appendChild(anchor); } const sourceButton = renderSourceButton(source); if (sourceButton) { parent.appendChild(sourceButton); } }");
            html.AppendLine("    function render() { const node = nodes.get(current); document.getElementById('node-title').textContent = node ? node.name : '(empty)'; const content = document.getElementById('content'); clear(content); const choices = document.getElementById('choices'); clear(choices); if (!node) return; node.lines.forEach(line => { const p = document.createElement('p'); p.className = line.kind === 'Metadata' ? 'line meta' : 'line'; if (line.speaker) { const speaker = document.createElement('span'); speaker.className = 'speaker'; speaker.textContent = line.speaker + '：'; p.appendChild(speaker); } p.appendChild(text(line.text)); renderAnchor(p, line.anchor, line.source); content.appendChild(p); }); node.choices.forEach(group => { if (group.prompt) { const prompt = document.createElement('p'); prompt.className = 'meta'; prompt.appendChild(text(group.prompt)); renderAnchor(prompt, group.anchor, group.source); choices.appendChild(prompt); } group.options.forEach(option => { const row = document.createElement('div'); const button = document.createElement('button'); button.className = 'choice'; button.textContent = option.text + ' -> ' + option.target; button.onclick = () => go(option.target); row.appendChild(button); renderAnchor(row, option.anchor, option.source); choices.appendChild(row); }); }); if (node.defaultNext) { const row = document.createElement('div'); const button = document.createElement('button'); button.className = 'choice'; button.textContent = 'Continue -> ' + node.defaultNext; button.onclick = () => go(node.defaultNext); row.appendChild(button); renderAnchor(row, null, node.source); choices.appendChild(row); } document.getElementById('path').textContent = path.join(' / '); }");
