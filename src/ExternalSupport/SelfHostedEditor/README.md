...
- Preview reading mode can switch between `Static` and `Flow`. Static keeps the full active block visible; Flow starts from the title, advances one line per click, fades speaker names in quickly, types newly revealed body text, then reveals the full choice group at once.
