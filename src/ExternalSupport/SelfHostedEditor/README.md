...
Workspace summary migration has started: `WorkspaceSummaryHostedModelBuilder` is the preferred normal path, while `ProjectWorkspaceDraftSummaryModelBuilder` owns the explicit `workspace-summary-status` fallback. Keep future summary counts sourced from hosted Compiler / Tooling / LanguageServer / Runtime payloads instead of expanding draft extraction.
