...
`check:workspace-fs` verifies the workspace path and write target guards used by backend file boundaries: only workspace-relative paths are accepted, absolute paths and traversal are rejected, resolved paths must stay under the workspace root, valid paths must match the explicit write target catalog, and directory-only targets are not treated as file writes.
