...
`check:workspace-fs` verifies the workspace path guard used by backend file boundaries: only workspace-relative paths are accepted, absolute paths and traversal are rejected, resolved paths must stay under the workspace root, and valid paths still pass through the write target whitelist.
