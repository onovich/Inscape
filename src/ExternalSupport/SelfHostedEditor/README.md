...
- Localization table that can review against a selected previous CSV, filter visible rows by review status, keep session-local translation overrides, export a lightweight draft CSV, and export a real updated localization CSV through the shared CLI update flow.
