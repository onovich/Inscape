...
Editor-to-preview sync currently has three modes through `inscape.preview.sourceSyncMode`: `off`, `click`, and `selection`. On choice lines, the option label before `->` participates in preview reveal hit testing, while the target title after `->` remains normal definition navigation. `selection` means the source-editor text selection, not preview-side clicking; it only posts lightweight `revealSource` messages to already-open preview panels and must not open panels implicitly or trigger a preview re-render on every caret move.
