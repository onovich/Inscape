...
- `Entries/LanguageServerEntry.cs` exposes `--definition-project <root> <nodeName> [--override source.inscape temp.inscape]` and `--references-project <root> <nodeName> [--override source.inscape temp.inscape]` for project-level node navigation. These probes reuse `DslScriptSourcesLoaderDomain` and `StoryGraphCompilerDomain` instead of VSCode workspace scanning.
