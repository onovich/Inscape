...
namespace Inscape.Tooling {
    ...
    public static class PreviewHtmlRendererDomain {
        ...
        static string RenderSerializedOutput(object viewModel, JsonSerializerOptions jsonOptions, PreviewStyleSheetModel? styleSheet) {
            ...
            html.AppendLine("    function sourcePayload(source) { return source && source.sourcePath ? { sourcePath: source.sourcePath, line: Math.max(0, (source.line ?? 1) - 1), column: Math.max(0, (source.column ?? 1) - 1) } : undefined; }");
            html.AppendLine("    function editorSourcePayload(source) { return source && source.sourcePath ? { sourcePath: source.sourcePath, line: Math.max(0, (source.line ?? 0)), column: Math.max(0, (source.column ?? source.character ?? 0)) } : undefined; }");
