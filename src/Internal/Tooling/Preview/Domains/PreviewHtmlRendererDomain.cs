    ...
    public static class PreviewHtmlRendererDomain {
        ...
        static string RenderSerializedOutput(object viewModel, JsonSerializerOptions jsonOptions, PreviewStyleSheetModel? styleSheet) {
            ...
            html.AppendLine("    :root { color-scheme: light dark;"
                ...
                + " --inscape-query-interpolation-background: " + Css(style.QueryInterpolationBackground) + ";"
                + " --inscape-query-interpolation-text-color: " + Css(style.QueryInterpolationTextColor) + ";"
            ...
            html.AppendLine("    .query-interpolation { display: inline-flex; align-items: center; vertical-align: baseline; border-radius: 6px; padding: 0 0.28em; margin: 0 0.04em; background: var(--inscape-query-interpolation-background); color: var(--inscape-query-interpolation-text-color); font-family: \"Cascadia Mono\", Consolas, monospace; font-size: 0.88em; line-height: 1.45; }");
            ...
            html.AppendLine("    function sourcePayload(source) { return source && source.sourcePath ? { sourcePath: source.sourcePath, line: Math.max(0, (source.line ?? 1) - 1), character: Math.max(0, (source.column ?? 1) - 1) } : undefined; }");
            html.AppendLine("    function editorSourcePayload(source) { return source && source.sourcePath ? { sourcePath: source.sourcePath, line: Math.max(0, (source.line ?? 0)), character: Math.max(0, (source.character ?? source.column ?? 0)) } : undefined; }");
            html.AppendLine("    function openSource(source) { const payload = editorSourcePayload(source); if (!payload || !vscode) return; vscode.postMessage({ type: 'openSource', source: payload }); }");
            ...
            html.AppendLine("    function sourceMatches(candidate, target) { const payload = sourcePayload(candidate); return !!payload && !!target && sameSourcePath(payload.sourcePath, target.sourcePath) && payload.line === Math.max(0, (target.line ?? 0)); }");
            ...
            html.AppendLine("    function appendPreviewText(parent, value) { const raw = String(value ?? ''); const pattern = /\\[([A-Za-z_][A-Za-z0-9_]*(?:\\.[A-Za-z_][A-Za-z0-9_]*)*)\\]/g; let cursor = 0; let match = pattern.exec(raw); while (match) { if (match.index > cursor) { parent.appendChild(text(raw.slice(cursor, match.index))); } const token = document.createElement('span'); token.className = 'query-interpolation'; token.textContent = match[0]; token.title = 'Host query interpolation'; parent.appendChild(token); cursor = match.index + match[0].length; match = pattern.exec(raw); } if (cursor < raw.length) { parent.appendChild(text(raw.slice(cursor))); } }");
