...
- manual-review 候选可执行 `Preview Apply` / `Apply`，经由共享 CLI `apply-node-map-candidate-project` 得到同一 apply result；SelfHostedEditor UI 要求二次确认。dev-host HTTP 路径仍只更新可下载 node map payload；Electron desktop 路径会先通过 `workspace.write-back-backup` 备份，再经 `stable-node-map.write-sidecar` 写回 sidecar。

SelfHostedEditor 仍缺：

- VSCode 式的本地 `.review-backup.json` / revert 文件恢复体验。SelfHostedEditor desktop 路径已经有 `.inscape-workspace/backups/` 写前备份，但尚未复制 VSCode 的专用 revert 文件 UI。
...
| Stable node map update/review | 有命令与 review UI，apply/dry-run 经由共享 CLI，revert 是宿主备份恢复 | 已有 Node Map 入口、共享 report、source jump、sidecar 下载；manual-review apply/dry-run 经由共享 CLI；dev-host 为 download-ready，Electron desktop 可确认后备份并写回 sidecar；未复制本地 revert 文件 UI | 已对齐核心闭环，Round 9 继续复核边界 |
...
9. 已下沉 Stable Node Map candidate apply 为共享 Tooling / CLI 动作；VSCode 与 SelfHostedEditor 都已改为调用共享命令。SelfHostedEditor dev-host 阶段只更新可下载 node map payload；Electron desktop 阶段可在 `Confirm Apply` 后通过 `workspace.write-back-backup` + `stable-node-map.write-sidecar` 写回 node map sidecar，但仍不复制 VSCode 的专用 revert 文件 UI。
