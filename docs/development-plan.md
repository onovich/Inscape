...
- ExternalSupport 架构包含外部平台支持：`VSCode` 与 `UnityPlugin`
...
- 当前 `src/ExternalSupport/VSCode` 已完成 B 阶段拆分，`Scripts/ExtensionManifestEntry.js` 主要保留注册入口、实例装配和少量 glue
...
目标：把 `Scripts/ExtensionManifestEntry.js` 从单文件入口拆成可维护模块。
...
- `Scripts/ExtensionManifestEntry.js` 变为注册入口。
- VSCode 作者体验不回归。
