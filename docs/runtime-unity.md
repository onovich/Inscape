...
调研入口：`D:\UnityProjects\Bird\Assets\Resources_Runtime`。后续接手者应优先阅读 `StorySystem` 和 `DirectorSystem`，再决定 Adapter 输出边界。
