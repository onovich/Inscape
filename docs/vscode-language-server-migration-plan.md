...
`src/ExternalSupport/VSCode` 当前已经按 `Commands`、`DslScript`、`EditorAuthoring`、`HostBinding`、`HostSchema`、`Preview`、`Styles` 与 `Entries` 拆分。它仍然是一个轻量 JS 前端：诊断借道 CLI，节点、speaker、host binding、metadata、Host Schema query / event 等作者提示由扩展侧轻量扫描或 CLI capability endpoint 支撑。
