...
>
> 2026-05-17 块语法补充：Compiler 已支持 `# 标题` 节点声明、中文标题跳转、项目内重复标题诊断，以及标题前缺空行的 info 级 style hint。`:: node.name` 已退出 Compiler 当前语法；VSCode 侧旧扫描、高亮和 snippet 将在 Goal 0 后续节点继续删除。

- 叙事结构采用图模型。节点可以形成链、树、回环和一般有向图，不强制单向流。
- 块级叙事单元必须使用显式节点标题，便于跳转、复用、调试、图视图和协作讨论。
- 第一版多文件项目内的节点标题全局唯一，跨文件跳转不需要 `include`。
- 第一版项目入口使用节点内 metadata `@entry` 声明；未声明时兼容回退到按文件路径排序后的第一个节点，并产生提示诊断。
- 项目级 CLI 支持 `--entry 标题` 临时覆盖入口，方便调试任意节点；这不会修改源文件，也不会替代 `@entry` 作为项目默认入口声明。
...
这类写法更接近 Markdown 与文章结构；后续仍需补 stable node id 落盘实现。
...
- 项目级 CLI 可通过 `--entry 标题` 临时指定入口。缺失的覆盖入口会产生 `INS034`；非法入口标题会产生 `INS033`。
...
2. 继续清理 `:: node.name` 在 VSCode 和旧文档中的残留入口，当前 Compiler 已不再解析旧块语法。
