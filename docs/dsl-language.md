...
>
> 2026-05-17 块语法补充：Compiler 已支持 `# 标题` 节点声明、中文标题跳转、项目内重复标题诊断，以及标题前缺空行的 info 级 style hint。旧 `:: node.name` 仍兼容；VSCode 高亮、补全、跳转、Outline 和 `Inscape: Insert Node Title` 自动 `_01` 编号已跟进。
