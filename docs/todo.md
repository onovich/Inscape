...
- 2026-05-26 进行中：SelfHostedEditor L10N 视图正在接 `LocalizationAlignmentReportModel.Presenter`。当前未提交草案新增 `/api/localization-review` 开发宿主桥、`SelfHostedEditorLocalizationReviewBridge` 和 presenter 驱动的 L10N 表格渲染；它复用 `update-node-map-project`、`extract-l10n-project`、`audit-l10n-alignment-project`，前端只消费 presenter，不复制候选评分。已通过 `check:syntax` / `check:structure` / `check:model`；最小脚本 HTTP smoke 已通过，完整 `samples/court-loop.inscape` 的 `/api/localization-review` smoke 仍会超时，需优先定位 full sample 路径。该草案还修了 dev server 所有 JSON request body 的 UTF-8 BOM 剥离。
- 当前执行顺序（2026-05-26）：
