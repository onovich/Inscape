...
- 已完成：Preview 增加 `Static` / `Flow` 阅读模式切换。Static 保持当前完整 block 展示；Flow 从标题开始，点击预览区逐行放出正文，新出现的 speaker 快速淡入，正文使用打字机效果；正文结束后一次性显示全部选项，并同时显示选项文本与目标标题。Flow 滚轮导航只在预览面板自身滚到顶部 / 底部后接管：向上按阈值撤回上一步，向下按阈值快进一步；选项可见时禁止向下快进。
