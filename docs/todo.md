...
- [ ] 下一轮：P5 Round 3 Mock query model。按 [P5 SelfHostedEditor Runtime Authoring Goal 模式执行指南](self-hosted-editor-p5-goal-mode-execution-guide.md) 从 Host Schema `queries[]` 生成 editor-session-only mock query authoring model，覆盖 string / number / bool mock value、missing / invalid / unknown query 的结构化提示；不要把 mock query 写入正式 Runtime state，也不要扩展 Host Schema action policy。
