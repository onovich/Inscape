# TODO

状态：持续维护

本文件记录已经能执行或需要调研的工作。仍未形成共识的问题放在 [待确认问题](open-questions.md)，已经形成长期决策的问题放在 [ADR](adr/README.md)。

## 阶段 1：DSL 与轻工具链

- [ ] 准备一个 3 到 5 分钟的图叙事样例，最好包含复入、回环和多出口选择。
- [ ] 用 Yarn-like、Ink-like、Ren'Py-like、Inscape-like 四种写法重写同一片段，比较阅读感、解析复杂度和 IR 映射成本。
- [ ] 定义第一版最小语法：显式节点、对白、旁白、选项、跳转、注释、元信息。
- [ ] 定义节点名规范：字符集、层级分隔符、跨文件唯一性、重命名策略。
- [ ] 定义行级隐式 hash 的输入、规范化规则、版本号和碰撞处理。
- [ ] 设计 Narrative Graph IR 的 JSON 草案。
- [ ] 设计源映射格式，覆盖节点、行、选项、跳转和诊断。

## VSCode 支持

- [ ] 设计 `.inscape` 文件扩展名和语言 ID。
- [ ] 编写 TextMate 语法高亮草案，弱化元信息并凸显剧情文本。
- [ ] 设计 Language Server 能力范围：补全、诊断、跳转定义、引用查找、大纲、悬浮说明。
- [ ] 设计补全数据来源：当前文件节点、项目节点、角色表、资源别名、宿主 Schema。
- [ ] 定义第一版诊断清单：重复节点、缺失目标、不可达节点、空选项、孤立节点、循环提示。

## HTML 调试预览

- [ ] 设计无引擎预览的最小 UI：当前节点、文本、选项、路径、诊断和锚点。
- [ ] 决定预览载体：静态 HTML、VSCode WebView，还是共享包同时支持。
- [ ] 定义预览输入：直接读取 IR，还是通过 Compiler Core 增量服务获取。
- [ ] 支持节点回环、重开、返回上一步和路径记录。
- [ ] 显示行级 hash、本地化 key 和源位置，方便调试本地化与存档定位。

## Unity / Bird 适配调研

- [ ] 梳理 Bird `TalkingTM` 与 Inscape Node/Line/Edge 的字段映射。
- [ ] 梳理 Bird `L10N_Talking` 当前 `talkingId + index` 模型与行级 hash 模型的迁移方式。
- [ ] 调研 `StorySystem` 是否可以直接消费 Narrative Graph IR，而不是必须生成 ScriptableObject。
- [ ] 调研 Unity Adapter 输出格式：JSON、二进制、ScriptableObject、CSV，或多格式。
- [ ] 深入调研 `DirectorSystem` / `TimelineEffectTM`：判断 Timeline 是外部演出资源、节点 Hook，还是未来 Presentation IR。
- [ ] 设计 Timeline 引用的第一版最小表达方式，但不让 DSL 直接变成演出时间轴语言。

## 变量与状态查询，第二版前置调研

- [ ] 对比 Yarn、Ink、Ren'Py、Twine 的变量、函数和宿主 API 边界。
- [ ] 设计表达式只表达数据查询的模型，不在 DSL 中绑定具体业务实体或服务端。
- [ ] 设计宿主查询 Schema：谓词名、参数类型、返回类型、同步/异步、错误策略。
- [ ] 明确查询表达式是否允许副作用。当前倾向是不允许。
