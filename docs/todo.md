...
- [ ] 继续 Host Integration Package CLI Round 3 Existing Artifact Assembly：复用现有 `compile-project` / `inspect-usage-project` / `inspect-host-schema-project` / `audit-host-integration-project` / `extract-l10n-project` 生产逻辑，把 graph、usage、host schema capabilities、host integration audit 与 l10n 写入 package 目录并更新 manifest status；仍禁止 Host Bridge candidate generator / generated apply / runtime / host save / policy 扩张。
