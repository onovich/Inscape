...
- [ ] 下一轮：P5 Round 10 Substate preview save/load。按 [P5 SelfHostedEditor Runtime Authoring Goal 模式执行指南](self-hosted-editor-p5-goal-mode-execution-guide.md) 与 [P5 Round 10 Substate Preview Save/Load Goal 模式执行指南](self-hosted-editor-p5-round10-substate-goal-mode-execution-guide.md) 执行，最多 3 轮会话完成；只展示 / 导入导出 Runtime substate，用于编辑器预览和测试，不把 substate 扩张为完整 host save，也不保存宿主业务状态、完整 Log、完整 action history 或完整 Trace Replay。
