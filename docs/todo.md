...
1. 按 [/goal 后续目标计划](goal-plan.md) 推进 Goal 6：Host Schema endpoint 已建立 LanguageServer / Tooling 契约，下一步把 VSCode query / event provider 迁到 LanguageServer 优先路径，再移除 JS direct JSON fallback。
...
- [ ] 按 [VSCode LanguageServer Migration Plan](vscode-language-server-migration-plan.md) 继续推进 Host Schema capability endpoint：LanguageServer `--host-schema-capabilities-project` 已复用 Tooling 契约；下一步让 VSCode query / event provider 优先调用 LanguageServer，并在稳定后移除 JS provider 的重复 JSON fallback。
