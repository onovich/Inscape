...
- [ ] 继续 Host Integration Partner Readiness Round 3：输出 source location external contract 与 localization anchor export contract。该阶段只做 contract / fixture / report / planning；不得自动进入完整 host save、Unity / Bird / Host SDK、Rollback / Trace Replay / Flashback、Presentation IR、Runtime Preview Bridge、Sinan Runtime Integration 或 Host Schema action policy 扩张。
