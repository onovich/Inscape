...
1. **再推进 Stable Node ID 主线。**
	...
	- 下一步建议顺序：
		- 已推进：标题重命名审查已有 review item 列表、candidate 跳转、node map / raw report 入口，以及 manual-review 项的显式 `Apply candidate stable id`。下一步可再评估是否需要 dry-run / revert / multi-apply。
