...
- [ ] 下一轮：P5 Round 7 Runtime status surface。按 [P5 SelfHostedEditor Runtime Authoring Goal 模式执行指南](self-hosted-editor-p5-goal-mode-execution-guide.md) 展示 current node、visible choices、visible step count、provider、pending action、Runtime error 与 query provider 来源；继续让状态来自 Runtime payload，不在 SelfHostedEditor 里重算 Runtime state。
