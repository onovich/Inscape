...
- 当前执行顺序（2026-06-01）：
	...
	5. 第四实施节点整理 Editor Backend 会话边界：Runtime 与 line-map 已完成第一层 `sessionId` 状态边界；后续继续把 LanguageServer、localization baseline/update 这些开发服务器 + CLI 临时 workspace 桥逐步收成更接近桌面客户端的会话模型。短期可保留 HTTP dev bridge，但前端不得新增语义真相。
