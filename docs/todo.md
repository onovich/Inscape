...
- [ ] 实现 autosave / flush / recovery：手动 Save 已可写盘并清理 recovery snapshot，dirty edit 已写入 snapshot 且下次打开可扫描；仍需接 idle autosave、close/switch/app-exit flush、recovery restore / discard / later 动作与 GUI recovery smoke。
