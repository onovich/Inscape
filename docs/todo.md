...
状态：持续维护，P5 SelfHostedEditor Runtime authoring / productization 已通过 final validation；Host Integration Partner Readiness 已完成 final validation，结论为 `Host Integration Partner Readiness: PASS`，见 [Host Integration Partner Readiness Final Validation Report](host-integration-partner-readiness-final-validation-report.md)。`Host Integration Package CLI` 已完成 final validation，结论为 `Host Integration Package CLI: PASS`，见 [Host Integration Package CLI Final Validation Report](host-integration-package-cli-final-validation-report.md)。`Host Integration Readiness Report Generator` 已完成 final validation，结论为 `Host Integration Readiness Report Generator: PASS`，见 [Host Integration Readiness Report Generator Final Validation Report](host-integration-readiness-report-generator-final-validation-report.md)。`Host Integration Static Artifact POC Partner Handoff Kit` 已完成 final validation，结论为 `Host Integration Static Artifact POC Partner Handoff Kit: PASS`，见 [Host Integration Static Artifact POC Partner Handoff Final Validation Report](host-integration-static-artifact-poc-partner-handoff-final-validation-report.md)。`Host Bridge Candidate Generator First Slice` 已完成 final validation，结论为 `Host Bridge Candidate Generator First Slice: PASS`，见 [Host Bridge Candidate Generator Final Validation Report](host-bridge-candidate-generator-final-validation-report.md)。Sinan 仍只作为 partner profile / fixture / dry-run planning 输入。下一候选方向必须由用户批准；不要自动进入 confirmed Host Bridge、POC-2 catalog projection、generated apply、Sinan Runtime Integration、Runtime Preview Bridge、Unity / Host SDK、Rollback / Trace Replay / Flashback、Presentation IR、完整 host save 或 Host Schema action policy 扩张。
...
- [x] Round 2 partner feedback schema / fixture：新增 [Static Artifact POC Partner Feedback Schema](host-integration-static-artifact-poc-partner-feedback-schema.md)、[partner-feedback.generic.json](host-integration-static-fixtures/partner-feedback.generic.json) 与 [Round 2 Audit](host-integration-static-artifact-poc-partner-feedback-audit.md)，明确 partner evidence / candidate evidence / confirmed truth 分层。
- [x] Round 3 handoff smoke / docs hardening：新增 [PartnerHandoffKitSmoke.js](host-integration-static-fixtures/PartnerHandoffKitSmoke.js) 与 [Round 3 Audit](host-integration-static-artifact-poc-partner-handoff-smoke-audit.md)，并同步 static fixture / package CLI / readiness report smoke 入口。
- [x] Round 4 final validation / PASS-FAIL closure：输出 [Host Integration Static Artifact POC Partner Handoff Final Validation Report](host-integration-static-artifact-poc-partner-handoff-final-validation-report.md)，同步 handoff / TODO / README，结论为 `Host Integration Static Artifact POC Partner Handoff Kit: PASS`。

## 2026-06-22 Host Bridge Candidate Generator First Slice

目标：在不写 confirmed Host Bridge、不 generated apply、不做 POC-2 catalog projection 的前提下，把既有 Host Integration Package artifact 转成 review-only Host Bridge Candidate 生成闭环。

- [x] 输出 Host Bridge Candidate Generator First Slice 的独立 goal-mode execution guide。2026-06-22 见 [Host Bridge Candidate Generator First Slice Goal 模式执行指南](host-bridge-candidate-generator-goal-mode-execution-guide.md)；执行预算为 6 轮：baseline / command contract、shared Tooling candidate domain、CLI command / output guard、smoke / docs hardening、buffer / compatibility closure、final validation。
- [x] Round 1 baseline / command contract / source-of-truth audit：新增 [Host Bridge Candidate Generator Baseline Audit](host-bridge-candidate-generator-baseline-audit.md)，注册 `generate-host-bridge-candidate-package` commands/help，并保持 direct execution guarded until shared Tooling generation lands。
- [x] Round 2 shared Tooling candidate domain / package reader integration：新增 `src/Internal/Tooling/HostBridgeCandidate/` 和 [Host Bridge Candidate Generator Domain Audit](host-bridge-candidate-generator-domain-audit.md)，覆盖 `empty` / `ready` / `blocked` / `invalid` / `incompatible`，并保持 candidate-only / no-host-write 边界。
- [x] Round 3 CLI command / diagnostics / output guard：`generate-host-bridge-candidate-package <package-dir> -o <candidate.json>` 已接入 shared Tooling domain，新增 [Host Bridge Candidate Generator CLI Audit](host-bridge-candidate-generator-cli-audit.md)，覆盖 output guard、missing/invalid/unsupported package、no BOM、determinism 与 candidate-only 输出。
- [x] Round 4 smoke fixture / determinism / docs hardening：新增 [HostBridgeCandidateGeneratorSmoke.js](host-integration-static-fixtures/HostBridgeCandidateGeneratorSmoke.js) 与 [Host Bridge Candidate Generator Smoke Audit](host-bridge-candidate-generator-smoke-audit.md)，覆盖真实 package export + candidate generation、blocked unknown schema evidence、determinism、no BOM、output guard、package export no-candidate default 与 no host writes。
- [x] Round 5 buffer / edge-case hardening / compatibility closure：新增 [Host Bridge Candidate Generator Compatibility Audit](host-bridge-candidate-generator-compatibility-audit.md)，readiness report 只读汇总已有 candidate evidence，candidate generator 忽略无关 optional readiness report 破损，并补充 internal tests / readiness smoke coverage。
- [x] Round 6 final validation / PASS-FAIL closure：新增 [Host Bridge Candidate Generator Final Validation Report](host-bridge-candidate-generator-final-validation-report.md)，完整最终验证矩阵与边界扫描通过，结论为 `Host Bridge Candidate Generator First Slice: PASS`。
...
P2.5 Host Schema / Host Bridge / Unity-Bird adaptation
  ...
  -> P5 active: SelfHostedEditor Runtime authoring/productization (guide: self-hosted-editor-p5-goal-mode-execution-guide.md)
...
- [x] 完成 P1 Round 10 embedded invoke transport skeleton：新增 `SelfHostedEditorPreloadBackendTransport` 与 `check:preload-transport`，`EditorBackendClient` 可在 preload API 存在时自动使用 desktop transport，否则保留 HTTP dev transport。
...
- [x] 完成 P1 Round 28 flush rules：新增 text-free flush plan，覆盖 manual Save / close window / switch workspace / app exit 的 latest revision flush request、unsafe target blocking 和 save failure visible UI state。
- [x] 完成 P1 Round 29 recovery snapshot：新增 recovery snapshot plan，dirty buffer 生成含文本 snapshot write payload，status / cleanup request 保持 text-free，save success 生成 cleanup request。
...
- [x] 接入真实 Electron recovery restore / discard / later 动作：新增 desktop-only `recovery.restore` / `recovery.discard` / `recovery.later` commands；restore 校验 snapshot path/hash/text 后写回 `.inscape` 并清理 snapshot，discard 删除 snapshot，later 保留 snapshot 且只更新当前 session action state；相关响应保持 text-free。
- [x] 补真实 Electron GUI recovery smoke：`smoke:desktop-gui-recovery` 启动真实 BrowserWindow，加载 app protocol Workbench 和 sandbox-compatible `ElectronPreload.cjs`，通过 renderer preload API 覆盖 open folder、read、edit、manual save、idle autosave、recovery restore / later / discard，以及 diagnostics / completions 使用 restore 后当前 buffer 的真实 IPC/main-process 路径。
- [x] 补真实 Windows packaged GUI smoke：`smoke:desktop-package-gui` 运行 `dist/win-unpacked/Inscape SelfHostedEditor.exe`，由 packaged app 自己加载 app protocol Workbench、打开临时 workspace、编辑保存、recovery restore，并验证 diagnostics / completions 使用 restore 后当前 buffer。
...
- [x] 继续推进 Host Bridge 作为 Inscape 可读 ID 到宿主 ID / 资源 / handler / query implementation 的映射层；Host Schema 只描述能力清单，不承担资源映射。2026-06-17 边界收口见 [SelfHostedEditor P2.5 Boundary Closure Audit](self-hosted-editor-p2-5-boundary-closure-audit.md)；Unity GUID / asset path / Bird ID 只允许在 Host Bridge / adapter artifact / 外部项目配置中出现。
- [x] Unity 支持层仍作为 ExternalSupport / 独立插件方向评估，不让 UnityEngine / UnityEditor 依赖进入 `Internal`。P2.5 只修 `src/ExternalSupport/UnityPlugin` 的 Bird importer 原型，不创建通用 Unity package，不新增顶层 `Scripts` / `Resources`。
- [x] 完成 P2.5 Round 3 真实 Timeline 绑定导出链路。2026-06-17 审计结果见 [SelfHostedEditor P2.5 Timeline Export Audit](self-hosted-editor-p2-5-timeline-export-audit.md)；主样例导出 1 个 `talking.exit` hook，phase fixture 导出 4 个 hook 用于 Dry Run 验证 unsupported phase warning。
...
- [x] 完成 P3 Round 6 `audit-host-integration-project` 最小实现。2026-06-18 审计结果见 [SelfHostedEditor P3 Host Integration Audit](self-hosted-editor-p3-host-integration-audit.md)；新增 `Internal/Tooling/HostIntegrationAudit` 与 CLI 命令，对账 Usage Manifest、Host Schema capability、Host Bridge capability，报告 unknown query/action、legacy event、参数 mismatch、missing bridge id / handler。
...
- [x] 完成 P4 final validation / PASS-FAIL 收口。2026-06-18 最终报告见 [SelfHostedEditor P4 Final Validation Report](self-hosted-editor-p4-final-validation-report.md)；最终矩阵、边界扫描、P4 PASS 门槛与文档接力入口均已收口，下一候选阶段只能是 P5 SelfHostedEditor Runtime authoring / productization。
- [x] 输出 P5 SelfHostedEditor Runtime authoring / productization goal 模式执行指南。2026-06-18 指南见 [P5 SelfHostedEditor Runtime Authoring Goal 模式执行指南](self-hosted-editor-p5-goal-mode-execution-guide.md)；约束 16 轮内完成，1-12 轮主线、13-15 轮缓冲、16 轮最终验收，每轮必须 Debug 自检、架构自检、验证通过后提交推送，再进入下一轮。
- [x] 完成 P5 Round 1 baseline audit / authoring contract。2026-06-18 审计见 [SelfHostedEditor P5 Baseline Audit](self-hosted-editor-p5-baseline-audit.md)，合同见 [SelfHostedEditor P5 Runtime Authoring Contract](self-hosted-editor-p5-runtime-authoring-contract.md)；确认现有 Runtime bridge、Preview、Host Schema / Host Binding、transport、session cache 入口，固定 P5 最小 fixture、缺口、风险和 per-round smoke 策略。本轮未改 Runtime / Compiler / Unity / Bird 或产品行为。
- [x] 完成 P5 Round 2 Runtime authoring session contract。2026-06-18 审计见 [SelfHostedEditor P5 Runtime Session Audit](self-hosted-editor-p5-runtime-session-audit.md)；新增 text-free Runtime authoring session model 与 `check:model` 契约检查，区分 current snapshot、formal state、P4 substate、pending action、log entries、branch evidence、provider / stale / error metadata，并保持 dev-host HTTP 与 desktop command 为薄 transport。
- [x] 完成 P5 Round 3 Mock query model。2026-06-18 审计见 [SelfHostedEditor P5 Mock Query Model Audit](self-hosted-editor-p5-mock-query-model-audit.md)；新增 editor-session-only mock query authoring model 与 `check:model` 契约检查，覆盖 string / number / bool ready value、missing / invalid / unknown / unsupported 的结构化提示，并保持 mock query 不写入 formal Runtime State、P4 substate 或 Host Schema action policy。
- [x] 完成 P5 Round 4 Mock query UI。2026-06-18 审计见 [SelfHostedEditor P5 Mock Query UI Audit](self-hosted-editor-p5-mock-query-ui-audit.md)；新增 Host view mock query 编辑表面、reset / apply to runtime preview、unavailable / invalid / unknown 状态、dev-host `queryProvider` 透传与 preload whitelist 覆盖，UI 只消费 authoring model，不复制 Runtime 条件求值或 query evaluator。
- [x] 完成 P5 Round 5 Action capability / pending surface。2026-06-18 审计见 [SelfHostedEditor P5 Action Authoring Audit](self-hosted-editor-p5-action-authoring-audit.md)；新增 Host view Runtime Actions 面板、Host Schema `actions[]` / Host Bridge handler mapping 对账、Runtime action request / pending 摘要、debug resume controls、dev-host `--action-dispatcher` / `--resume-action` 桥接与 Preview pending guard，继续不复制 Runtime action dispatcher 或 Host action policy。
...
- [x] 完成 P5 Round 10 Substate preview save/load。2026-06-19 审计见 [SelfHostedEditor P5 Substate Authoring Audit](self-hosted-editor-p5-substate-authoring-audit.md)，执行记录见 [P5 Round 10 Substate Preview Save/Load Goal 模式执行指南](self-hosted-editor-p5-round10-substate-goal-mode-execution-guide.md)；Host view 新增 Runtime Substate panel，可导出当前 Runtime Preview substate、validate 粘贴的 JSON artifact，并且只在 `compatible` 时 import 恢复 Runtime Preview；dev-host HTTP / backend command / preload whitelist 已接入，embedded desktop backend 显式返回 unavailable，不伪造实现。
- [x] 完成 P5 Round 11 Error / empty / stale state hardening。2026-06-20 审计见 [SelfHostedEditor P5 Error State Audit](self-hosted-editor-p5-error-state-audit.md)；新增 Runtime error-state inventory contract 与 Host view `Runtime States` 面板，统一聚合 Preview、Runtime Status、Mock Query、Runtime Actions、Log / Backlog、Branch Receipts 与 Runtime Substate 的 ready / empty / unavailable / error / stale / blocked 状态，diagnostic category 覆盖 schema / bridge / query / action / runtime-cli / transport / session / script / payload，并保持 workspace text、host payload、mock value table、完整 Runtime snapshot / substate / log / action history 不外泄。
- [x] 完成 P5 Round 12 integration smoke + docs closure。2026-06-20 审计见 [SelfHostedEditor P5 Integration Audit](self-hosted-editor-p5-integration-audit.md)；新增 `check:runtime-authoring-integration`，通过真实 dev-host HTTP 串起 Host Schema / Host Bridge catalog、session-only Mock Query、Runtime Preview、`fire` action、`wait` pending / debug resume、Runtime Log、Branch Receipts、Runtime Substate export / validate / import 与 Runtime States 总览，并覆盖 Runtime unavailable、missing schema / bridge / handler、empty log / branch / substate、script drift blocked import 和 payload diagnostic。
- [x] 完成 P5 final validation / PASS-FAIL 收口。2026-06-20 报告见 [SelfHostedEditor P5 Final Validation Report](self-hosted-editor-p5-final-validation-report.md)；最终验证矩阵和边界扫描均通过，结论为 `P5 SelfHostedEditor Runtime authoring / productization: PASS`。
- [x] 完成 Post-P5 下一方向决策门。2026-06-20 brief 见 [Post-P5 Next Direction Decision Brief](post-p5-next-direction-decision-brief.md)；已比较 Unity / Host SDK、高级运行时调试、Presentation IR / 跨引擎 / 独立 Runtime、Host Bridge / Schema 自动化等候选方向，并建议优先批准 Host Bridge / Host Schema 自动化与代码生成收口的 scoping first slice。
- [x] 批准 Host Integration Partner Readiness 作为下一短阶段。2026-06-21 决策见 [Host Integration Partner Readiness 决策简报](sinan-cooperation/host-integration-partner-readiness-decision-brief-2026-06-21.md)；Sinan 可作为第一批真实 partner profile / fixture，但不得成为 Inscape core dependency。
- [x] 输出商务反馈口径。2026-06-21 见 [Host Integration Partner Readiness 商务反馈口径](sinan-cooperation/host-integration-partner-readiness-business-response-2026-06-21.md)；对外口径为 Static Artifact POC planning: GO，Sinan Runtime Integration / Runtime Preview Bridge / Hard Dependency / Sinan-specific Core Semantics: HOLD 或 NO。
- [x] 输出 Host Integration Partner Readiness 的独立 goal-mode execution guide。2026-06-21 见 [Host Integration Partner Readiness Goal 模式执行指南](host-integration-partner-readiness-goal-mode-execution-guide.md)；执行预算为 6 轮：4 轮主工作、1 轮缓冲 / 收口、1 轮 final validation。
- [x] 完成 Host Integration Partner Readiness Round 1 Baseline Audit / Artifact Inventory。2026-06-21 审计见 [Host Integration Partner Readiness Baseline Audit](host-integration-partner-readiness-baseline-audit.md)，integration package 初稿见 [Host Integration Package Contract](host-integration-package-contract.md)；本轮确认现有 compile / host schema / usage / audit / localization 输出可作为 POC-1 静态 artifact 基线，Sinan 仍只作为 partner profile / fixture。
- [x] 完成 Host Integration Partner Readiness Round 2 Integration Package + Narrative Graph IR External Contract。2026-06-21 [Host Integration Package Contract](host-integration-package-contract.md) 已收口最小 package 目录、manifest、artifact status 和静态边界；新增 [Narrative Graph IR External Contract](narrative-graph-ir-external-contract.md)，定义外部 importer 可依赖字段、禁依赖字段、source location 连接点和版本兼容规则。
- [x] 完成 Host Integration Partner Readiness Round 3 Source Location + Localization Anchor Export Contract。2026-06-21 新增 [Source Location External Contract](source-location-external-contract.md) 与 [Localization Anchor Export Contract](localization-anchor-export-contract.md)，并同步 [Host Integration Package Contract](host-integration-package-contract.md) 的 `source-map/source-locations.json` / `localization/anchor-map.json` 关系；本轮未实现 package command、Host Bridge candidate generator 或任何 runtime integration。
- [x] 完成 Host Integration Partner Readiness Round 4 Host Bridge Candidate Contract + Static Artifact Fixtures。2026-06-21 新增 [Host Bridge Candidate Contract](host-bridge-candidate-contract.md)、[Host Integration Partner Readiness Fixtures](host-integration-partner-readiness-fixtures.md) 和 [Round 4 static fixture pack](host-integration-static-fixtures/fixtures.json)，覆盖 minimal dialogue、branching、localization、missing speaker、unknown action、unsupported feature、source diagnostic；本轮未实现 candidate generator、generated apply、Host SDK 或任何 runtime integration。
- [x] 完成 Host Integration Partner Readiness Round 5 Static Artifact Smoke / POC Planning / Buffer。2026-06-21 新增 [Host Integration Readiness Report Contract](host-integration-readiness-report-contract.md)、[Host Integration Static Artifact Smoke](host-integration-static-artifact-smoke.md)、[Host Integration Partner Readiness POC-1 Checklist](host-integration-partner-readiness-poc-1-checklist.md) 与 [Sinan Static Artifact POC Planning Note](sinan-cooperation/sinan-static-artifact-poc-planning-note.md)；本轮只做 contract / fixture / report / planning，未进入完整 host save、Unity / Bird / Host SDK、Rollback / Trace Replay / Flashback、Presentation IR、Runtime Preview Bridge、Sinan Runtime Integration 或 Host Schema action policy 扩张。
- [x] 完成 Host Integration Partner Readiness Round 6 final validation / PASS-FAIL closure。2026-06-21 最终报告见 [Host Integration Partner Readiness Final Validation Report](host-integration-partner-readiness-final-validation-report.md)；验证矩阵、static artifact smoke、docs 入口扫描和边界扫描均通过，结论为 `Host Integration Partner Readiness: PASS`。
- [x] 输出 Host Integration Package CLI 的独立 goal-mode execution guide。2026-06-22 见 [Host Integration Package CLI Goal 模式执行指南](host-integration-package-cli-goal-mode-execution-guide.md)；执行预算为 6 轮：baseline / command contract、package domain / manifest writer、existing artifact assembly、source-map / anchor-map / minimal report、smoke / docs、final validation。
- [x] 完成 Host Integration Package CLI Round 1 Baseline / Command Contract。2026-06-22 审计见 [Host Integration Package CLI Baseline Audit](host-integration-package-cli-baseline-audit.md)；`export-host-integration-package-project <workspace> -o <out-dir>` 已进入 `commands` / `help` 并提供 skeleton guard，当前不写 package。
- [x] 完成 Host Integration Package CLI Round 2 Package Domain / Manifest Writer。2026-06-22 审计见 [Host Integration Package CLI Manifest Audit](host-integration-package-cli-manifest-audit.md)；新增 `Inscape.Tooling` package manifest model、artifact index writer、path guard 与 manifest-only export writer，CLI 仅调 shared domain，capability flags 全部保持 `false`。
- [x] 完成 Host Integration Package CLI Round 3 Existing Artifact Assembly。2026-06-22 审计见 [Host Integration Package CLI Artifact Assembly Audit](host-integration-package-cli-artifact-assembly-audit.md)；复用 compiler / Tooling shared domains 写出 graph、usage、host schema capabilities、host integration audit 与 l10n，并更新 manifest status。
- [x] 完成 Host Integration Package CLI Round 4 Source Copy / Source Map / Anchor Map / Minimal Report。2026-06-22 审计见 [Host Integration Package CLI Source Map Report Audit](host-integration-package-cli-source-map-report-audit.md)；复制 source `.inscape` 到 `source/`，生成 `source-map/source-locations.json`、`localization/anchor-map.json` 与最小 `reports/readiness-report.json`，并保持 readiness report 为静态 package 视角。
- [x] 完成 Host Integration Package CLI Round 5 Smoke Fixtures / Determinism / Docs。2026-06-22 审计见 [Host Integration Package CLI Smoke Determinism Audit](host-integration-package-cli-smoke-determinism-audit.md)；新增 `HostIntegrationPackageCliSmoke.js`，覆盖真实 CLI package export、artifact parse / structure、unknown action audit、compiler diagnostic、重复导出 byte-stable determinism、嵌套未知输出拒绝与 forbidden host candidate generation absence。
- [x] 完成 Host Integration Package CLI Round 6 Final Validation / PASS-FAIL Closure。2026-06-22 最终报告见 [Host Integration Package CLI Final Validation Report](host-integration-package-cli-final-validation-report.md)；最终验证矩阵、package CLI smoke、VSCode baseline check、`git diff --check` 与边界扫描均通过，结论为 `Host Integration Package CLI: PASS`。
- [x] 输出 Host Integration Readiness Report Generator 的独立 goal-mode execution guide。2026-06-22 见 [Host Integration Readiness Report Generator Goal 模式执行指南](host-integration-readiness-report-generator-goal-mode-execution-guide.md)；执行预算为 5 轮：baseline / report generator contract、shared report domain / package reader、CLI command / diagnostics aggregation、fixtures / smoke / docs hardening、final validation。
- [x] 完成 Host Integration Readiness Report Generator 阶段。2026-06-22 最终报告见 [Host Integration Readiness Report Generator Final Validation Report](host-integration-readiness-report-generator-final-validation-report.md)；新增独立 package readiness report generator `generate-host-integration-readiness-report-package <package-dir> -o <report.json>`，并让 package export 复用同一个 shared Tooling report generator。新增 `HostIntegrationReadinessReportSmoke.js` 覆盖真实 package、missing artifact、invalid JSON、output guard、determinism 和 boundary flags；最终验证矩阵与边界扫描均通过。本阶段未进入 Host Bridge candidate generator、Static Artifact POC partner handoff、POC-2 catalog projection、generated apply、Sinan Runtime Integration、Runtime Preview Bridge、Unity / Host SDK、完整 host save、Rollback / Trace Replay / Flashback、Presentation IR 或 Host Schema action policy 扩张。
- [x] 输出 Static Artifact POC Partner Handoff Kit 的独立 goal-mode execution guide。2026-06-22 见 [Static Artifact POC Partner Handoff Kit Goal 模式执行指南](host-integration-static-artifact-poc-partner-handoff-goal-mode-execution-guide.md)；执行预算为 4 轮：handoff kit contract / exchange workflow、partner feedback schema / fixture、handoff smoke / docs hardening、final validation。本阶段已获用户批准，generic first，Sinan 只作为 partner profile，不提交生成 package，不做 Host Bridge candidate generator、POC-2 catalog projection、generated apply、Runtime / Host SDK 或完整 host save。
- [x] 执行 Static Artifact POC Partner Handoff Kit 阶段，按 4 轮 guide 完成 partner handoff kit、feedback schema / fixture、handoff smoke、docs closure 和 final validation；结论为 `Host Integration Static Artifact POC Partner Handoff Kit: PASS`。
- [x] 输出 Host Bridge Candidate Generator First Slice 的独立 goal-mode execution guide。2026-06-22 见 [Host Bridge Candidate Generator First Slice Goal 模式执行指南](host-bridge-candidate-generator-goal-mode-execution-guide.md)；执行预算为 6 轮：baseline / command contract、shared Tooling candidate domain、CLI command / output guard、smoke / docs hardening、buffer / compatibility closure、final validation。本阶段已获用户批准，candidate 仍只是 review evidence，不写 confirmed Host Bridge、不做 generated apply、不进入 POC-2 catalog projection。
- [x] 执行 Host Bridge Candidate Generator First Slice 阶段，按 6 轮 guide 完成 standalone package candidate generator、shared Tooling domain、CLI command、smoke、docs closure 和 final validation；见 [Host Bridge Candidate Generator Final Validation Report](host-bridge-candidate-generator-final-validation-report.md)，结论为 `Host Bridge Candidate Generator First Slice: PASS`。
...
- [ ] 外部合作线后续必须重新获得用户批准；不得把 Sinan Runtime、Sinan 专用 DSL、Sinan data layout 或 hard dependency 写入 Inscape core。
...
- [x] desktop backend v0 基础稳定后，推进 backend 管理的 workspace-scoped long-lived LanguageServer：P1.5 已完成 workspace state、当前 buffer override、崩溃重启 / fallback、状态可观测、packaged artifact 与 semantic parity 验收；后续只保留非阻塞 hardening。
...
- [x] 第 2 轮：完成 F2 Outline fallback 降级为 explicit unavailable fallback。`SelfHostedEditorDocumentSymbolBridge` 现在只在 LanguageServer document-symbol 请求失败时使用 draft outline；`LanguageServerDocumentSymbolModelMapper` 会拒绝 malformed shared payload，`DocumentOutlineController` 显示 `LanguageServer outline error` 而不是回退草模。`check:syntax`、`check:structure`、`check:model`、`check:semantic-parity-http` 已通过。
- [x] 第 3 轮：完成 F3 Preview fallback 显式 offline mode。`PreviewPanelController` 现在为 Runtime / Compiler / offline draft / contract-error 路径标记 provider 状态并渲染轻量状态文本；draft document model 只在没有 `compiler-project` graph 时构建，malformed Compiler graph 继续显示 `Preview data error` 且不回退草模。`check:syntax`、`check:model`、`check:style-structure`、`check:runtime-http`、`check:semantic-parity-http` 已通过。
- [x] 第 4 轮：完成 F4 StoryGraph fallback 显式 offline mode。`StoryGraphPreviewController` 现在区分 `compiler-project`、`offline-draft` 与 `contract-error`，Graph 面板显示对应状态；malformed compiler-project graph payload 会显示 `Graph data error`，不再回退 draft graph。`check:syntax`、`check:model`、`check:structure`、`check:node-map-http`、`check:semantic-parity-http` 已通过。
- [x] 第 5 轮：完成 F5 Localization hosted / draft table 分离。`LocalizationReviewRowsModelBuilder` 现在只消费 `provider: "localization-review"` 的 Tooling presenter，空 hosted presenter 保持空 hosted review 而不混入 draft rows；review-unavailable 才进入 draft fallback。updated CSV export / linked-file replace 会检查 hosted provider，draft fallback 模式禁用真实 update / replace 并显示原因。`check:syntax`、`check:structure`、`check:model`、`check:localization-review-http`、`check:localization-update-http` 已通过。
- [x] 第 6 轮：完成 Backend Session v0 project-session model 与 status 投影。新增 `EditorBackendProjectSessionModel` 与 `SelfHostedEditorProjectSessionContractCheck`，`EditorBackendClient.projectSession.status()` 现在把 `/api/session-cache-status` 投影为 `inscape.self-hosted-editor.project-session`，明确 `mode: "dev-host"`、共享 project session id、workspace request-snapshot active path / document count / revision，以及 LanguageServer `process-per-request`、Runtime / line-map / localization `bounded-cache` 子 session。Runtime、line-map、localization bridge 已统一使用同一个 backend session id；sidebar session panel 显示 backend mode 与 session id。`check:syntax`、`check:structure`、`check:model`、`check:session-cache-http`、`check:runtime-http`、`check:line-map-http`、`check:localization-update-http` 已通过。
- [x] 第 7 轮：完成 LanguageSession v0 request model session-aware。新增 `EditorBackendLanguageSessionRequestModel`，`EditorBackendClient.languageSession.*` 现在为 diagnostics / completions / definition / references / hover / documentSymbols 统一补 `sessionId`、active document path、document revision 与窄 query envelope，同时展开为兼容现有 dev-host `/api/*` 的 flat payload；底层仍明确保持 `process-per-request`。`check:syntax`、`check:structure`、`check:model`、`check:semantic-parity-http`、`check:references-http`、`npm --prefix src\ExternalSupport\VSCode run check:semantic-parity` 已通过。
- [x] 第 8 轮：完成 optional long-lived LanguageServer session spike，默认路径保持可回退。新增 `SelfHostedEditorLanguageSessionBridge` 与 `check:language-session`，可通过 `SELF_HOSTED_EDITOR_LANGUAGE_SESSION=stdio` 让 dev host 对 diagnostics / documentSymbols 尝试复用 `Inscape.LanguageServer --stdio`；失败会回退到原 process-per-request helper，server close 会 dispose shared session，默认路径不变。`check:syntax`、`check:structure`、`check:model`、`check:language-session`、默认 `check:semantic-parity-http`、stdio 开关下 `check:semantic-parity-http` 已通过。
- [x] 第 9 轮：完成阶段大验收与 README / architecture / handoff / backend migration map / TODO 收口。文档已明确 `project-session` 与 `language-session-request` 只是 dev-host mode 的迁移词汇，不代表正式 desktop backend 已完成；fallback 收口状态已同步到 handoff 和 architecture plan。完整验证已通过：`dotnet build Inscape.slnx --no-restore`、Internal tests、VSCode manifest `node --check`、VSCode `check:structure` / `check:semantic-parity`，以及 SelfHostedEditor `check:syntax`、`check:structure`、`check:style-structure`、`check:model`、`check:language-session`、`check:semantic-parity-http`、`check:runtime-http`、`check:line-map-http`、`check:localization-review-http`、`check:localization-update-http`、`check:node-map-http`、`check:session-cache-http`、`check:references-http`、`check:host-schema-http`、`check:host-binding-http`、`check:static-assets-http`。
- [x] 第 10 轮：完成最终审计。未发现 Localization 或 LanguageSession 验收暴露的新边界问题；生产 `Scripts/` 下直接 `fetch()` 只剩默认样例静态文件加载，业务 `/api/*` 仍由 backend client / transport 承担。最终复查将 round 9 文档状态从“第 8 轮已推进”改为“10 轮已完成”，并保留正式 desktop backend 另起计划的边界。
...
- [x] 第 4 轮：将 session 意味最强的 Runtime、line-map、Localization bridge 迁到 `EditorBackendClient`。`SelfHostedEditorRuntimeBridge`、`SelfHostedEditorLineMapBridge`、`SelfHostedEditorLocalizationReviewBridge` 保持原有 UI-facing API、sessionId 和 fallback 行为，但不再直接 `fetch("/api/...")`；`SelfHostedEditorFeatureBootstrapper` 为三者共享同一个 backend client。`check:structure` 已新增守卫，防止这些 session bridge 回退到直接 dev-host API fetch；本轮通过 Runtime、line-map、Localization review/update 与 session-cache direct/HTTP smoke。
- [x] 第 5 轮：将 LanguageServer-backed authoring bridge 迁到 `EditorBackendClient`。Diagnostics、Completion、Definition、Hover、References、DocumentSymbols bridge 保持原有 mapper 和 fallback 返回形状，但请求改走 `backendClient.languageSession.*`；`SelfHostedEditorFeatureBootstrapper` 为这些 authoring bridge 注入同一个 backend client。`check:structure` 已守住这些 bridge 不再直接 `fetch("/api/...")`；本轮通过 SelfHostedEditor `check:semantic-parity-http`、`check:references-http` 与 VSCode `check:semantic-parity`。
- [x] 第 6 轮：将剩余 Host Schema、Host Binding、StoryGraph、Stable Node Map bridge 迁到 `EditorBackendClient`。`SelfHostedEditorHostSchemaBridge`、`SelfHostedEditorHostBindingBridge`、`SelfHostedEditorStoryGraphBridge`、`SelfHostedEditorStoryNodeMapBridge` 继续保持 mapper/cache/fallback 语义不变，只通过 `hostCapabilities`、`storyGraph` 与 `stableNodeMap` 业务入口调用现有 dev-host API。`check:structure` 现在全局禁止 `Scripts/` 下直接 `fetch("/api/...")`；本轮通过 Host Schema/Binding direct+HTTP、Node Map direct+HTTP 和 semantic parity HTTP smoke。
