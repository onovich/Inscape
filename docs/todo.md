...
- [ ] 下一轮：P5 Round 5 Action capability / pending surface。按 [P5 SelfHostedEditor Runtime Authoring Goal 模式执行指南](self-hosted-editor-p5-goal-mode-execution-guide.md) 增加 action capability / pending / resume 调试表面；继续只消费 Host Schema `actions[]` 与 Runtime snapshot 摘要，不复制 action dispatcher 或 Host Bridge handler 语义。
