...
- 2026-05-26 进行中：SelfHostedEditor L10N 视图正在接 `LocalizationAlignmentReportModel.Presenter`。当前 `/api/localization-review` 开发宿主桥、`SelfHostedEditorLocalizationReviewBridge` 和 presenter 驱动的 L10N 表格渲染已落地；它复用 `update-node-map-project`、`extract-l10n-project`、`audit-l10n-alignment-project`，前端只消费 presenter，不复制候选评分。已通过 `check:syntax` / `check:structure` / `check:model`；后续复查表明完整 `samples/court-loop.inscape` 的底层 CLI 链路约 10 秒，因此下一步应优先盯 HTTP dev-host 传输和响应体，而不是继续怀疑 Tooling 算法本身。dev host 现已把返回 payload 收成精简 presenter，同时保留通用层已有的 `presenter.items` 契约以及 `summary` / `lineIdentity`。该草案还修了 dev server 所有 JSON request body 的 UTF-8 BOM 剥离。
...
- 当前执行顺序（2026-05-26）：
	...
	2. 第一实施节点转向 L10N 视图：先完成当前 `/api/localization-review` + presenter 渲染草案的真实 HTTP smoke 和提交；下一刀再从 session draft CSV 过渡到真实 CSV 文件选择 / 写回契约。优先复用 `LocalizationAlignmentReportModel.Presenter`，不要复刻 VSCode QuickPick 组织逻辑，也不要在前端 parser 上继续叠语义。
	3. 第二实施节点再推进 Preview Runtime Player：Runtime smoke 已经守住 `/api/runtime-state` / `/api/runtime-action` 的 compact payload 与 `choose` / `continue` 契约；下一刀把 `Static` / `Flow` 的当前节点、continue、choice 和历史路径映射到这条 Runtime 状态链路，而不是继续扩展前端 presenter 状态机。
	4. 第三实施节点整理 Editor Backend 会话边界：把 line-map、Runtime、LanguageServer 这些开发服务器 + CLI 临时 workspace 桥逐步收成更接近桌面客户端的会话模型；短期可保留 HTTP dev bridge，但前端不得新增语义真相。
