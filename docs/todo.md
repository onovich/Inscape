...
- [ ] 继续 Host Integration Package CLI Round 4 Source Copy / Source Map / Anchor Map / Minimal Report：复制 source `.inscape` 到 `source/`，生成 `source-map/source-locations.json`、`localization/anchor-map.json` 与最小 `reports/readiness-report.json`；仍禁止 partner-specific import 判断、Host Bridge candidate generator / generated apply / runtime / host save / policy 扩张。
