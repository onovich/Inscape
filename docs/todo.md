...
- [ ] 继续 Host Integration Package CLI Round 2 Package Domain / Manifest Writer：新增 shared Tooling domain、manifest model、artifact index writer 和 package path guard；固定 `runtimeIntegration = false`、`previewBridge = false`、`writesHostData = false`、`containsHostDependency = false`，仍禁止 Host Bridge candidate generator / generated apply / runtime / host save / policy 扩张。
