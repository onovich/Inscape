...
- [ ] 下一轮：P5 Round 4 Mock query UI。按 [P5 SelfHostedEditor Runtime Authoring Goal 模式执行指南](self-hosted-editor-p5-goal-mode-execution-guide.md) 与 [SelfHostedEditor P5 Mock Query Model Audit](self-hosted-editor-p5-mock-query-model-audit.md) 增加 SelfHostedEditor mock query 编辑表面，提供 reset / apply to runtime preview 入口和 unavailable / fallback / invalid 状态提示；UI 只消费 authoring model，不复制 Runtime 条件求值或 query evaluator。
