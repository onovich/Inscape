...
- [ ] 执行 Host Integration Readiness Report Generator 阶段：新增独立 package readiness report generator，建议命令为 `generate-host-integration-readiness-report-package <package-dir> -o <report.json>`，并让 package export 复用同一个 shared Tooling report generator。不要把本阶段扩成 Host Bridge candidate generator、Static Artifact POC partner handoff、POC-2 catalog projection、generated apply、Sinan Runtime Integration、Runtime Preview Bridge、Unity / Host SDK、完整 host save、Rollback / Trace Replay / Flashback、Presentation IR 或 Host Schema action policy 扩张。
