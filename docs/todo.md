...
- [ ] 下一轮：P5 Round 11 Error / empty / stale state hardening。按 [P5 SelfHostedEditor Runtime Authoring Goal 模式执行指南](self-hosted-editor-p5-goal-mode-execution-guide.md) 与 [P5 Round 11 Error / Empty / Stale State Goal 模式执行指南](self-hosted-editor-p5-round11-error-state-goal-mode-execution-guide.md) 继续推进，最多 3 轮会话完成；聚焦 Runtime unavailable、empty session、stale snapshot、malformed artifact、payload contract error 与错误恢复状态，不混入完整 host save、Unity / Bird / Host SDK、Rollback / Trace Replay / Flashback。
