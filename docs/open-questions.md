...
7. Host Bridge 如何设计？
   ...
   - 已确认 Host Schema 是统一能力清单，包含 `queries[]` 与 `actions[]`；第一版最小字段不包含 rollback / replay / receipt / failure / timeout policy。仍需细化的是现有 `events[]` 到 `actions[]` 的兼容 / 迁移口径，以及 Usage / audit 输出格式。
