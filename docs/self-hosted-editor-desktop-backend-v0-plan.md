...
- 2026-06-17 P1 post-40 已补真实 Electron workspace open / read / write buffer 局部闭环：main process 持有 `ElectronWorkspaceSessionStore`，通过原生 open-folder 选择目录，扫描真实 `.inscape` 文件并读入 `DocumentBufferStore`；`project-session.status` / workspace list / update draft / save 响应保持 text-free，显式 `document-buffer.read` 才返回请求文档正文。`document-buffer.save` / `save-all` 已写回真实磁盘并覆盖 stale revision / disk conflict；autosave flush、recovery 和 GUI edit-save-recovery smoke 仍未完成。
