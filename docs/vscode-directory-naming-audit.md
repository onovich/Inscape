...
| `WorkspaceIndex` | 需改 | `Workspace` 不应作为类型名前缀，`Index` 只是二级限定；当前只剩 HostBinding 与 HostSchema capability | 拆为 `HostBinding/Providers`、`HostSchema/Providers` |
...
6. 再拆剩余 `WorkspaceIndex`，优先按 `HostBinding`、`HostSchema` 分业务。
