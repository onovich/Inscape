...
| post-40 B3 | autosave / flush / recovery IO | 部分完成。dirty draft update 已在 main process 写入 `.inscape-workspace/recovery/<relative>.snapshot.json`，open workspace 会扫描 text-free recovery status，正常保存后清理对应 snapshot；Electron session store 已提供 `runAutosave()` / `flushDirtyDocuments()`，可执行既有 autosave / flush plan 并通过真实 save 路径写盘、清理 recovery。`ElectronWorkspaceLifecycle` 已把 idle autosave timer、BrowserWindow close、workspace switch 和 app-exit flush 接到共享 session store。`check:electron-workspace` 覆盖真实 snapshot 正文写盘、状态不泄露正文、重开扫描、篡改路径跳过、waiting autosave 不写盘、ready autosave 写盘和 app-exit flush 写盘；`check:electron-lifecycle` 覆盖 timer、close-window、switch-workspace、app-exit 与 text-free lifecycle status。restore / discard / later 动作与 GUI recovery smoke 仍待完成。 |
