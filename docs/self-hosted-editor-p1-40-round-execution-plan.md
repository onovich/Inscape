...
### 2026-06-17 Round 36：Windows internal package v0 / 等价本机启动 smoke

范围：确认当前 SelfHostedEditor desktop v0 的本机启动就绪度。当前 package 尚未引入 Electron runtime / builder，因此本轮采用“等价本机启动 smoke”收口：验证 package / lockfile、Desktop entry、Workbench entry、Electron autostart guard、preload whitelist 和 Round 35 最小闭环 smoke。本轮不生成 Windows installer、不新增 Electron 依赖、不执行真实 Electron GUI 启动。

完成内容：

1. 新增 `DevScripts/SelfHostedEditorDesktopStartupSmoke.js` 与 npm script `smoke:desktop-startup`。
2. startup smoke 验证 `package.json` / `package-lock.json`、Workbench document、Electron app entry、main process autostart guard、preload whitelist API。
3. startup smoke 生成 readiness 判断：当前 `electronRuntimeAvailable: false`，`windowsPackageGenerated: false`，known limitations 包含 `electron-runtime-not-installed` 与 `windows-package-not-generated`。
4. startup smoke 会复用 `smoke:desktop`，证明本机 contract 闭环仍能跑通。
5. `check:structure` 守住 `smoke:desktop-startup` script 入口与文件存在性。
...
1. 本轮没有把 Electron runtime / builder 临时塞进包，也没有把 package smoke 伪装成真实 Windows installer。
2. startup smoke 只验证本机启动 readiness 与 contract loop，真实 GUI / package 工作仍需后续引入 Electron runtime、packaging script 与手动或自动 GUI smoke。
...
### 2026-06-17 Round 38：Windows package script / electron-builder config contract

范围：补 Windows internal package 的可复现脚本与配置合同。引入 `electron-builder`、`package:windows` 与 `check:desktop-package`，但本轮不提交构建产物、不声明 package smoke 已完成、不打开 GUI，也不接真实 workspace 文件 IO。

完成内容：

1. `package.json` 新增 `main: "Desktop/ElectronMain.js"`，并新增 `package:windows` 指向 `electron-builder --win dir --x64`。
2. 新增 electron-builder build config：`appId`、`productName`、`asar`、`dist` 输出目录、Windows `dir` x64 target，以及 package files 白名单。
3. 新增 `DevScripts/SelfHostedEditorDesktopPackageContractCheck.js` 与 npm script `check:desktop-package`，验证 package entry、electron-builder 依赖、lockfile range、files 白名单、Windows target 与 artifact readiness。
4. `smoke:desktop-startup` 现在引用 package readiness：`windowsPackageScriptAvailable: true`，但在没有 `dist/win-unpacked/Inscape SelfHostedEditor.exe` 时继续保留 `windows-package-not-generated`。
5. `check:structure` 守住 `check:desktop-package`、`package:windows` 与 package contract 文件存在性。
...
1. package 配置仍只属于 `ExternalSupport/SelfHostedEditor`，没有把 electron-builder 或 Electron packaging 依赖带入 `Internal`。
2. package files 白名单只包含 `Desktop/`、`Resources/`、`Scripts/`、`package.json` 与 runtime 需要的 Monaco dependency，不把 `DevScripts/` 当产品文件打包。
3. `check:desktop-package` 只验证 package readiness，不伪造 artifact 通过；没有生成 package 时仍显式记录 `windows-package-not-generated`。
...
| 32 | write-back backup | 已完成。Round 32 先落 text-free backup plan；post-40 E 已把 localization CSV、`inscape.node-map.json`、`inscape.line-map.json` 写回前 backup 接到 Electron main process 真实复制与 retention cleanup，目标为 `.inscape-workspace/backups/`，默认启用。 |
...
| 后续轮次 | 目标 | 完成标准 |
|---|---|---|
| post-40 A | Electron IPC command boundary | 已完成。preload 内部通过固定 `inscape.self-hosted-editor.backend.invoke` channel 转发白名单 editor command；main dispatcher 复用 payload validator，拒绝未知 command，并让缺少实际 handler 的路径显式失败；`project-session.status` 返回 `embedded-desktop` 摘要；`check:electron-ipc`、Electron boundary / shell 与 runtime probe 覆盖该边界。 |
...
| post-40 B3 | autosave / flush / recovery IO | 已完成。dirty draft update 已在 main process 写入 `.inscape-workspace/recovery/<relative>.snapshot.json`，open workspace 会扫描 text-free recovery status，正常保存后清理对应 snapshot；Electron session store 已提供 `runAutosave()` / `flushDirtyDocuments()`，可执行既有 autosave / flush plan 并通过真实 save 路径写盘、清理 recovery。`ElectronWorkspaceLifecycle` 已把 idle autosave timer、BrowserWindow close、workspace switch 和 app-exit flush 接到共享 session store。desktop-only `recovery.restore` / `recovery.discard` / `recovery.later` 已接到 Electron main process 真实 IO：restore 写回 `.inscape` 并清理 snapshot，discard 删除 snapshot，later 保留 snapshot。`check:electron-workspace` 覆盖真实 snapshot 正文写盘、状态不泄露正文、重开扫描、篡改路径跳过、waiting autosave 不写盘、ready autosave 写盘、app-exit flush 写盘、restore 写盘、later 保留 snapshot 和 discard 删除 snapshot；`check:electron-lifecycle` 覆盖 timer、close-window、switch-workspace、app-exit 与 text-free lifecycle status；`smoke:desktop-gui-recovery` 覆盖 GUI 路径。 |
| post-40 C | GUI edit-save-recovery smoke | 已完成。`smoke:desktop-gui-recovery` 启动真实 Electron BrowserWindow，加载 app protocol Workbench 与 sandbox-compatible `ElectronPreload.cjs`，并通过 renderer preload API 覆盖 open folder、显式 read、edit、manual Save 写盘、idle autosave 写盘、发现 recovery、restore 写回、diagnostics / completions 使用 restore 后当前 buffer、later 保留 snapshot、discard 删除 snapshot，以及响应 text-free。 |
| post-40 D | Windows packaged GUI smoke | 已完成。新增 `smoke:desktop-package-gui`，在 `package:windows` 生成 unpacked package 后运行 `dist/win-unpacked/Inscape SelfHostedEditor.exe`，通过 packaged app 自身加载 Workbench/app protocol/preload，打开临时 workspace、显式 read、edit、manual Save 写盘、recovery restore 写盘、diagnostics / completions 使用 restore 后当前 buffer，并用结果文件确认 packaged exe 路径通过。 |
| post-40 E | 真实 write-back backup IO | 已完成。新增 desktop-only `workspace.write-back-backup` command、preload whitelist、`EditorBackendClient.workspace.writeBackBackup()` 与 Electron dispatcher；`ElectronWorkspaceSessionStore.runWriteBackBackup()` 扫描 `.inscape-workspace/backups/`、按 shared `EditorBackendWorkspaceBackupPlanModel` 复制 localization CSV / node-map / line-map 到 backup artifact，并删除 retention cleanup candidates。`check:electron-workspace` 覆盖三类 sidecar 真实复制、旧 backup 清理、disabled backup skip、unsupported `.inscape` skip、desktop-only route 与 text-free response。 |
| post-40 F | 真实 assets import IO | 已完成。新增 desktop-only `workspace.import-assets` command、preload whitelist、`EditorBackendClient.workspace.importAssets()` 与 Electron dispatcher；renderer payload 不允许传入 workspace 外 source path，Electron main process 通过原生多文件选择器或测试注入 selector 获得外部文件，复用 `EditorBackendWorkspaceAssetImportPlanModel` 生成 text-free copy plan，并复制图片 / 音频 / CSV 到 workspace `assets/images|audio|data`。`check:electron-workspace` 覆盖真实 image/audio/CSV 复制、重名后缀、unsupported skip、取消导入、缺失源失败不留下目标文件、desktop-only route 与不持久化外部路径。 |
| post-40 G | 真实 GUI Preview smoke | 已完成。`smoke:desktop-gui-recovery` 与 `smoke:desktop-package-gui` 现在都会等待真实 Workbench handler ready，确认 `.story-preview` 渲染默认样例，点击 `查看证物` choice 后进入 `证物桌`，并通过 `.script-editor[data-active-source-line]` 验证编辑器 source reveal 到目标标题；Preview provider 仍显式区分 runtime/compiler/offline fallback，不把 fallback 伪装成 hosted success。 |
| post-40 H | 依赖安全 audit 收口 | 已完成。`monaco-editor` 保持 `0.55.1`，通过 npm `overrides` 将其间接 `dompurify` 从 `3.2.7` 提升到 `3.4.10`；未采用 `npm audit fix --force` 的 `monaco-editor@0.53.0` 降级路线。`npm audit` 已报告 0 vulnerabilities，GUI / packaged smoke 继续验证 Monaco/Electron 路径可用。 |
| P1.5 I | workspace-scoped long-lived LanguageServer 第一刀至第六刀 / final validation | 已完成。真实 Electron app 默认由 main process 创建 `Inscape.LanguageServer --stdio` session；open workspace 启动 / 复用，六个 language command 用当前 `DocumentBufferStore` snapshot 和 active-buffer override 进入同一个 stdio process，ProjectSession status 暴露 `kind: "long-lived"`、`health`、`lastError`、`documentRevisionLag`、`restartCount` 与 text-free fallback summary，且不暴露正文。workspace switch 会 dispose / restart，close-window 与 app-exit 成功 flush 后 dispose session；`check:electron-language-session` 覆盖真实 stdio process、dirty buffer override、六类 authoring endpoint、复用、revision lag、协议错误后 replacement process restart、switch restart 和 dispose；`check:electron-workspace` 断言 Electron language payload 保持 shared language-session envelope 与 query kind。packaged build 现在把 `Inscape.LanguageServer` artifact 打入 `resources/language-server`，packaged resolver 不回退源码树；`smoke:desktop-package-language` 启动生成后的 packaged app，并在无 fake language handler 的真实 long-lived path 上覆盖 diagnostics / completions / definition / references / hover / document symbols；`check:electron-language-fallback` 覆盖同一 artifact 的 `process-per-request` fallback。P1.5 final validation 已通过，后续 artifact/fallback 改进只属于 hardening，不再是 P1.5 blocker；P2 stable identity / localization review 可以作为下一 phase 进入。 |
