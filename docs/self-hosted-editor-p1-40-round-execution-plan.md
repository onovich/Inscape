...
| P1.5 I | workspace-scoped long-lived LanguageServer 第一刀 / 第二刀 / 第三刀 | 已完成。真实 Electron app 默认由 main process 创建 `Inscape.LanguageServer --stdio` session；open workspace 启动 / 复用，六个 language command 用当前 `DocumentBufferStore` snapshot 和 active-buffer override 进入同一个 stdio process，ProjectSession status 暴露 `kind: "long-lived"`、`health`、`lastError`、`documentRevisionLag`、`restartCount` 且不暴露正文。workspace switch 会 dispose / restart，close-window 与 app-exit 成功 flush 后 dispose session；`check:electron-language-session` 覆盖真实 stdio process、dirty buffer override、六类 authoring endpoint、复用、revision lag、协议错误后 replacement process restart、switch restart 和 dispose；`check:electron-workspace` 断言 Electron language payload 保持 shared language-session envelope 与 query kind。packaged app 内置 LanguageServer artifact 与 `process-per-request` 降级仍待后续容灾增强。 |
