...
| `src/ExternalSupport/VSCode/WorkspaceIndex/HostBindingProvider.js` | 识别 `@timeline...` 和 `[kind: alias]` completion / definition / hover 上下文 | `defer-behavior` | F1.3 先改 hover / completion 文案；行为继续保留 legacy fallback。 |
| `src/ExternalSupport/VSCode/WorkspaceIndex/DslScriptMetadataProvider.js` | hover 提示称 `@timeline ...` 是 host binding hint，`[kind: alias]` 是 inline equivalent | `migrate-tooling-copy` | F1.3 改成 `@timeline` 是事件挂载，bracket 只是 legacy inline host binding。 |
...
| `src/ExternalSupport/VSCode/README.md` | README 仍写 host binding alias completions cover `[kind: ...]` inline tag positions | `migrate-tooling-copy` | F1.3 改成 legacy inline host binding fallback。 |
...
- `src/ExternalSupport/VSCode/README.md`。
