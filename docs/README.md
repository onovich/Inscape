...
- 执行 Host Integration Package CLI goal：读 [Host Integration Package CLI Goal 模式执行指南](host-integration-package-cli-goal-mode-execution-guide.md) 和 [Host Integration Package CLI Baseline Audit](host-integration-package-cli-baseline-audit.md)，从 Host Integration Partner Readiness final validation 接上，新增 `export-host-integration-package-project <workspace> -o <out-dir>`，预算 6 轮；Round 1 已完成 command contract skeleton，下一轮进入 package domain / manifest writer。
