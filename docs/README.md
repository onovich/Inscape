...
- 执行 Host Integration Readiness Report Generator goal：读 [Host Integration Readiness Report Generator Goal 模式执行指南](host-integration-readiness-report-generator-goal-mode-execution-guide.md)，从 Host Integration Package CLI final validation 接上，新增独立 package readiness report generator，预算 5 轮；每轮必须 Debug 自检、架构自检、验证通过后提交推送，再进入下一轮。
