...
- [x] G10.4 将相似文本匹配作为人工候选输出，不静默继承旧译文。
  ...
  - [~] G10.4.2 继续调整 candidate scoring：sequence / context / line anchor 等信号更稳地影响 `changed` 与 `conflict` 分界；当前 report candidate 与 Tooling presenter 都已暴露 `rankPenalty`，Tooling presenter 也会在 review detail 中展示 current / candidate `lineId`。
