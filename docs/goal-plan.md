...
- [ ] G9.4 VSCode package 源码目录收口：`ExtensionEntry` 已收敛到 `Entries`，`PreviewWebview` 已收敛到 `Preview`，DslScript providers / diagnostics 已收敛到 `DslScript`；继续将剩余 `LanguageFeatures`、`WorkspaceIndex` 按 `EditorAuthoring`、`HostBinding`、`HostSchema` 拆分。
