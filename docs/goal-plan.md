...
- [ ] G9.4 VSCode package 源码目录收口：`ExtensionEntry` 已收敛到 `Entries`，`PreviewWebview` 已收敛到 `Preview`，DslScript providers / diagnostics 已收敛到 `DslScript`，EditorAuthoring providers 已收敛到 `EditorAuthoring`，HostBinding / HostSchema providers 已收敛到各自业务目录，`LanguageFeatures` / `WorkspaceIndex` 过渡目录已删除；仍需完成命名规范尾部自检。
  - [x] G9.4.1 `PreviewRevealBridge` 迁入 `Preview/Bridges`，删除根级 `Bridges` 目录。
  - [ ] G9.4.2 审视 `Styles`：确认并迁入 `EditorAuthoring` / `Preview`，或建立合规业务主语。
  - [ ] G9.4.3 审视根级 `Commands`：按业务归位，或明确 VSCode command entry 例外。
  - [ ] G9.4.4 审视 `StyleDefaults.js` 等无规范后缀文件名，补齐角色后缀或拆分职责。
