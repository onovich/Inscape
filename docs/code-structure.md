...
4. `src/ExternalSupport/VSCode/` 已成为 VSCode 前端源码位置，且入口层已收敛到 `Entries`，预览层已收敛到 `Preview`，DslScript 作者体验已收敛到 `DslScript`，作者工具通用 provider 已收敛到 `EditorAuthoring`，HostBinding / HostSchema 作者提示已收敛到各自业务目录；`Commands`、`Styles` 仍是当前主要源码目录，资源已收敛到 `Resources`。
