...
src/
  ...
  ExternalSupport/
    VSCode/                     当前 VSCode 前端扩展，已按 Entries / Commands / LanguageFeatures 等目录完成 B 阶段拆分
    UnityPlugin/
...
4. `src/ExternalSupport/VSCode/` 已成为 VSCode 前端源码位置，且入口层已收敛到 `Entries`；`Commands`、`LanguageFeatures`、`WorkspaceIndex`、`Bridges`、`PreviewWebview`、`Styles` 仍是当前主要源码目录，资源已收敛到 `Resources`。
