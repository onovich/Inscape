...
补充判断：Inky 本身不是 Inscape 编辑器体验的充分参照。它证明了“边写边玩”的价值，但没有充分提供现代编程编辑器应有的导航、补全、诊断与多视图能力。Inscape 的编辑器方向应更靠近“编程编辑器 + Notion/Medium 写作体验 + 叙事生产工具”的组合。
...
| 编辑器 | 后续目标是脚本视图 + 节点图视图 + CSV 视图，体验上参考编程编辑器与 Notion/Medium |
| Timeline | 第一版只做宿主资源引用，后续再判断是否演化为 Presentation IR 或姊妹工具 |
...
- Inky 的即时预览值得借鉴，但其编辑体验并不构成 Inscape 的上限；Inscape 需要明显更强的 VSCode 级工作流与多视图能力。
...
- 不把缩进作为核心语义载体；如果需要结构辅助，应优先通过块标题、跳转和工具视图解决。

需要继续调研的语法问题：

- Timeline hook 应使用行内标签、块级 metadata，还是独立 hook 行。
- 是否从 `:: node.name` 迁移到 `# 标题` 风格块语法，以及两者能否并存。
...
- VSCode WebView 预览方案。
- 节点重命名 / 迁移表 / 本地化模糊匹配设计。
- 本地化 CSV、宿主绑定 CSV、宿主事件清单三类表的边界定义。
...
- 多视图编辑器基础：脚本 / 节点图 / CSV 三视图切换与互相定位。
...
- AI、Skill、GitHub 集成如何服务创作与审校，而不是成为噪声。
