...
每完成一个功能、重构或文档决策节点，都必须按同一顺序收口：

1. 立即做架构 / 目录 / 命名自检。
   - 对照 [编码与命名规范](coding-conventions.md) 检查 `Internal` / `ExternalSupport` 边界、`Layer / Business / Role / File` 目录公式、角色后缀、入口厚度和是否新增弱语义命名。
   - 尤其检查 VSCode command / provider / controller 是否重新吸收共享语义；若是，应优先下沉到 `Tooling` / `LanguageServer`，或把后续拆分节点写入计划。
2. 同步 TODO / handoff / goal-plan。
   - 已完成的小节点要勾选或改为 `[~]`，新的剩余风险要落入明确后续节点。
   - 长期架构或语义决策变更需要补 ADR 或更新现有契约文档。
3. 运行验证。
   - 至少执行本文“验证命令”中与本次改动相关的构建、测试和 Node 检查；无法执行的项必须在最终说明中写明。
4. 检查 diff。
   - 看 `git diff --stat`、关键 diff 和 `git status --short --branch`，确认没有生成物、临时文件或无关用户改动混入。
5. 提交并推送。
   - 验证通过后立即提交当前节点，并推送到远端。
   - 推送后再次确认分支状态；若仍有未提交改动，必须说明它们是否是本节点之外的既有改动。
