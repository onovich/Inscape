...
2. 本地化 Diff / Alignment 后续。
   ...
   - 当前实现已把更宽松的相似文本匹配限制在人工候选 / review report，不进入默认 `update-l10n` 确认译文；Goal 15 第一版已把 line sidecar refresh result / status / line id 信息接入 alignment audit。后续可以继续评估更强的 line identity 迁移契约、candidate diff 或编辑器 review UI。
