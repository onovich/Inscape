...
- CLI 当前已完成三步低风险瘦身：项目配置读取被提取到 `CliConfigLoader`，顶层元命令、单文件命令与项目级命令分支都已从 `CliCore` 主入口中抽离；项目扫描、override 与 UnitySample 项目命令辅助逻辑已集中到独立 support，避免继续把入口做大。下一步优先评估 `InscapeProjectService`，而不是把项目编排逻辑重新散回 CLI 或 VSCode。

- 2026-05-01 已完成 CLI 项目命令收口：新增 `CliProjectCommandSupport` 承接项目扫描、`--override`、UnitySample role/binding/export 辅助逻辑，`CliProjectCommandRunner` 统一改为 `switch` 分发；`CliCore` 删除了已失效的项目级重复实现与嵌套类型，当前入口主要负责参数分流、共享输出和退出码整合。验证通过：`dotnet build Inscape.slnx --no-restore`、`dotnet run --project tests\Inscape.Tests\Inscape.Tests.csproj --no-build`、`node --check tools\vscode-inscape\extension.js`。
...
- 只要改了 `tools/vscode-inscape/`，就必须 `npm run rebuild:vsix` 并安装；否则很容易把“旧扩展效果”误判成“新代码回归”。
...
2. Host Bridge 草案与 UnitySample 生成化：
...
9. 代码质量与入口重构：
   ...
   - 短期优先拆分测试文件、CLI command 分发和 VSCode extension 内部模块。
   - 当前 CLI 重构先从配置读取、路径归一化、输出辅助等纯工具职责开始，再逐步迁移命令 handler。
   - 中期引入 `InscapeProjectService` 作为工具链主入口，长期 Runtime Host 阶段再引入 `NarrativeRuntime`。
