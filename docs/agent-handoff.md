...
- Bird Adapter 原型：`export-bird-project` 可生成 `bird-manifest.json`、`L10N_Talking.csv` 和 `inscape-bird-l10n-map.csv`，支持 `--bird-role-map` 绑定 speaker 到 `roleId`，支持 `--bird-binding-map` 绑定资源 / Timeline 别名到 Bird 与 Unity 坐标，并支持 `--bird-existing-talking-root` 扫描现有 `talkingId` 避让冲突，详见 [Bird Adapter 原型](bird-adapter.md)。
...
1. Timeline Hook 最小表达：
   ...
   - 下一步设计 DSL 中如何引用 `hostBindings.alias`，只做宿主 hook，不把 DSL 变成演出时间轴语言。

2. Unity Editor Importer：
   - 读取 `bird-manifest.json` 的 `nodes`、`talkings`、`roles`、`hostBindings` 和 `localization`。
   - 创建或更新 `TalkingSO`，不要让 `Inscape.Core` 依赖 Unity。
