...
- Bird Adapter 原型：`export-bird-project` 可生成 `bird-manifest.json`、`L10N_Talking.csv` 和 `inscape-bird-l10n-map.csv`，并支持 `--bird-role-map` 绑定 speaker 到 `roleId`，详见 [Bird Adapter 原型](bird-adapter.md)。
