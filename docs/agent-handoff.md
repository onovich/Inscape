...
- Stable node map review 现在又补了一刀可操作闭环：manual-review 候选已支持显式 `Apply candidate stable id`，可直接把选中的旧 stable id 应用回 node map，并移除对应的重复临时项；同时会保存 `.review-backup.json`，支持 `Revert last applied stable id` 回退上一次应用结果。
