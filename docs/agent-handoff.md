...
- Unity/Bird Adapter。
- `bird-manifest.json`、Bird 兼容 L10N 导出和 Unity Editor Importer。
- 正式 Language Server。
- VSCode WebView 预览。
- 条件、变量、状态查询、自定义指令。
- 编辑器 Alpha。
...
1. Bird Adapter 输出协议：
   - 先设计 `bird-manifest.json` 的字段、版本和最小样例。
   - 明确 `anchor -> talkingId/index` 映射、节点入口 ID、角色/资源绑定和 ID 分配策略。
   - 短期推荐输出 manifest + Bird 兼容 `L10N_Talking.csv`，后续由 Unity Editor Importer 生成 `TalkingSO`。
...
Unity / Bird                       docs/bird-unity-research.md, docs/runtime-unity.md, docs/architecture.md, docs/todo.md
