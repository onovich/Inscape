...
1. Bird Unity 项目试跑 Importer：
   ...
   - Unity Editor Importer 原型已在 `tools/unity-bird-importer/`，下一步是在 Bird 项目内复制试跑；先执行 Dry Run，看 Console 与 `bird-import-dry-run-report.txt`，再检查 `.asset` diff、Addressables 和 L10N 合并风险。
