...
- 本轮会话已开始迁移 VSCode language features：`DslScriptCompletionProvider`、`DslScriptDefinitionProvider`、`DslScriptReferenceProvider` 与 `DslScriptHoverProvider` 已进入 `LanguageFeatures`，补全、定义跳转、引用查找和悬浮说明逻辑继续复用 `WorkspaceIndex` 中的 node / speaker / host binding / metadata provider 与 preview reveal bridge，不在编辑器层重建编译语义。
