...
- Localization review Quick Pick 已继续细化：主列表现在会带 `status / review / translation / source / candidate summary`，选中后还能继续在二级 Quick Pick 里比较候选并直接跳到 candidate 源位置；当前 Tooling presenter 会为超过两个的候选摘要显示 `+N more`，并提供 `show-candidate-diff` 二级动作，用 current / previous / translation / reason 摘要辅助人工审查，VSCode 只负责显示动作。
