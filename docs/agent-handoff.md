...
- 2026-05-01 已完成 CLI 项目命令收口：项目级命令分发位于 `CliProjectCommandRunner`，`.inscape` 项目源扫描/读取/override 被抽到 `CliDslSourceLoader`，UnitySample role/binding/export 辅助逻辑收口到 `CliUnitySampleSupport`；`CliCore` 只保留参数分流、共享输出和退出码整合。验证通过：`dotnet build Inscape.slnx --no-restore`、`dotnet run --project tests\Inscape.Tests\Inscape.Tests.csproj --no-build`、`node --check tools\vscode-inscape\extension.js`。
