...
- Bird Adapter 原型：`export-bird-project` 可生成 `bird-manifest.json`、`L10N_Talking.csv` 和 `inscape-bird-l10n-map.csv`，支持 `--bird-role-map` 绑定 speaker 到 `roleId`，支持 `--bird-binding-map` 绑定资源 / Timeline 别名到 Bird 与 Unity 坐标，支持 `@timeline alias` / `[timeline: alias]` 导出 `hostHooks`，并支持 `--bird-existing-talking-root` 扫描现有 `talkingId` 避让冲突，详见 [Bird Adapter 原型](bird-adapter.md)。
