...
- 2026-05-18 已推进 Goal 6.2 / G6.3：VSCode `HostSchemaCapabilityProvider` 已改为优先调用 LanguageServer `--host-schema-capabilities-project`，失败后回退 CLI `inspect-host-schema-project`；`[]` query interpolation 与 `@emit` host event provider 共用该 capability catalog。下一步建议 G6.4：删除 query / event provider 内的 JS direct JSON fallback，并补 output channel 日志。
