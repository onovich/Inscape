...
- Unity Editor Importer 原型：`tools/unity-bird-importer/Editor/InscapeBirdManifestImporter.cs` 可复制到 Bird 项目 `Assets/Editor/`，读取 manifest 并创建 / 更新 `TalkingSO`，将 Timeline Hook 映射为 `TalkingEffectTM.PlayTimeline`；已提供 `Dry Run Import Manifest...` 菜单和 `DryRunImportManifestFromCommandLine` batchmode 入口，输出创建 / 更新 / 缺失引用计划，报告既有 `TalkingTM` 的字段级变化，并在 manifest 同目录写入带 Inscape 节点、锚点和源位置的 `bird-import-dry-run-report.txt`。详见 [Unity Editor Importer 草案](unity-editor-importer.md)。
- Bird 项目 batchmode Dry Run：2026-04-29 已在 `D:\UnityProjects\Bird` 通过 Unity 2023.2.22f1 执行 `DryRunImportManifestFromCommandLine`。样例 manifest 计划创建 5 个 `TalkingSO`、0 个 warning、0 个 Timeline Hook。本次只 Dry Run，没有创建 `.asset`。Bird 项目当前新增 `Assets/Editor/InscapeBirdManifestImporter.cs` 及 `.meta`，尚未提交。

尚未落地：

- Bird 项目内的真实导入试跑与结果复盘。
...
1. Bird Unity 项目真实 Import 试跑：
   ...
   - Unity Editor Importer 原型已复制到 Bird 的 `Assets/Editor/` 并完成 batchmode Dry Run。下一步是决定是否增加真实 Import 的命令行入口，或在 Unity 菜单执行 `Import Manifest...`。重点检查生成 `.asset` diff、`nextTalking` 引用、选项目标、Addressables 和 L10N 合并风险。
