...
- VSCode 原型：TextMate 高亮、snippets、诊断桥接、节点补全、角色补全、宿主绑定别名补全、Outline、跳转定义、引用查找、Hover、block CodeLens、本地化导出/更新命令。角色补全会读取 `inscape.config.json` 中的 `unitySample.roleMap`，并回退扫描工作区已有 speaker；角色 Ctrl+Click 会跳到 role map 对应行，Find All References 会列出工作区对白；block 标题 CodeLens 显示 `入边` / `出边`，用于追溯调用方和跳转被调用方；宿主绑定提示会读取 `unitySample.bindingMap`，覆盖 `@timeline ...` 和 `[kind: ...]` 位置。
...
1. VSCode 可玩预览视图：
   - 用户希望继续优化 VSCode 体验，并在当前脚本编写阶段加入预览视图，让开发者能像玩文字游戏一样体验当前游戏流程。
   - 当前 CLI / HTML 预览已能消费项目级 IR，支持节点跳转、选择、回环、Back、Restart、路径和锚点显示。
   - 下一步建议把这套能力接入 VSCode WebView，优先复用 Core / CLI 编译结果，不在扩展中重写 parser 语义。
