...
- 本轮会话已开始迁移 VSCode language features：`DslScriptCompletionProvider`、`DslScriptDefinitionProvider` 与 `DslScriptReferenceProvider` 已进入 `LanguageFeatures`，补全、定义跳转和引用查找逻辑继续复用 `WorkspaceIndex` 中的 node / speaker / host binding / metadata provider 与 preview reveal bridge，不在编辑器层重建编译语义。
