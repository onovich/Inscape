...
- VSCode 原型：TextMate 高亮、snippets、诊断桥接、节点补全、角色补全、宿主绑定别名补全、Outline、跳转定义、引用查找、Hover、本地化导出/更新命令。角色补全会读取 `inscape.config.json` 中的 `bird.roleMap`，并回退扫描工作区已有 speaker；宿主绑定提示会读取 `bird.bindingMap`，覆盖 `@timeline ...` 和 `[kind: ...]` 位置。
