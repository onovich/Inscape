...
- Bird Adapter 原型：`export-bird-project` 可生成 `bird-manifest.json`、`L10N_Talking.csv` 和 `inscape-bird-l10n-map.csv`，支持 `--bird-role-map` 绑定 speaker 到 `roleId`，并支持 `--bird-existing-talking-root` 扫描现有 `talkingId` 避让冲突，详见 [Bird Adapter 原型](bird-adapter.md)。
...
1. Bird 宿主绑定设计：
   ...
   - 下一步需要资源别名到 Unity 资源、Timeline 名称到 `timelineId` 的绑定 Schema。
