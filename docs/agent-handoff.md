...
- Unity Editor Importer 原型：`tools/unity-bird-importer/Editor/InscapeBirdManifestImporter.cs` 可复制到 Bird 项目 `Assets/Editor/`，读取 manifest 并创建 / 更新 `TalkingSO`，将 Timeline Hook 映射为 `TalkingEffectTM.PlayTimeline`。详见 [Unity Editor Importer 草案](unity-editor-importer.md)。
...
1. Bird Unity 项目试跑 Importer：
   ...
   - Unity Editor Importer 原型已在 `tools/unity-bird-importer/`，下一步是在 Bird 项目内复制试跑，检查 `.asset` diff、Addressables 和 L10N 合并风险。
