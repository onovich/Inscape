...
1. Bird Unity 项目试跑 Importer：
   ...
   - Unity Editor Importer 原型已在 `tools/unity-bird-importer/`，下一步是在 Bird 项目内复制试跑；先执行 Dry Run，看 Console 与 `bird-import-dry-run-report.txt`。Dry Run 报告已经包含字段级变化，重点检查 `roleId`、`nextTalking`、`textDisplayType`、选项目标、`.asset` diff、Addressables 和 L10N 合并风险。
