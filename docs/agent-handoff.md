...
- 图叙事基线：显式节点名、跨文件项目编译、项目内节点名全局唯一、节点内 `@entry` 项目入口。
...
- VSCode 原型：TextMate 高亮、snippets、诊断桥接、节点补全、Outline、跳转定义、引用查找和 Hover。
...
1. VSCode 命令接入本地化：
   - 在 `tools/vscode-inscape/extension.js` 中添加命令。
   - 调用 `extract-l10n-project` 和 `update-l10n-project`。
   - 输出到用户选择的 CSV 路径或默认 `artifacts/l10n.csv`。

2. 项目入口覆盖策略：
   - 设计 CLI 参数或项目配置，让预览/编译可临时覆盖 `@entry`。
   - 目标是调试任意节点，而不是改变源文件。

3. Bird/Unity 调研：
   - 用户提供的参考项目在 `D:\UnityProjects\Bird\Assets\Resources_Runtime`。
   - 重点看 `StorySystem` 和 `DirectorSystem`。
...
- `@entry` 已实现，但项目入口覆盖策略还没设计。
- 行级 hash 已实现，但节点重命名迁移、显式稳定 ID 和模糊匹配还没做。
- VSCode 原型已经具备很多能力，但不是正式 Language Server。
