...
- 2026-05-12 已迁移 VSCode 前端源码：`tools/vscode-inscape` -> `src/ExternalSupport/VSCode`。到 2026-05-15，扩展内部已按 `Commands`、`WorkspaceIndex`、`LanguageFeatures`、`PreviewWebview`、`Styles`、`Bridges` 与入口层完成 B 阶段拆分；2026-05-18 入口层目录已从 `ExtensionEntry` 收敛为 `Entries`。验证入口为 `node --check src\ExternalSupport\VSCode\extension.js`。
