...
Host Bridge Candidate Generator First Slice 已完成 Round 5：buffer / edge-case hardening / compatibility closure。
- Round 5 审计见 [Host Bridge Candidate Generator Compatibility Audit](host-bridge-candidate-generator-compatibility-audit.md)。
- `HostIntegrationPackageReadinessReportDomain` 现在只读汇总已存在的 `host/host-bridge-candidate.json`，填充 `hostBridgeCandidate.status`、`candidateCount` 与 `writesHostData`；blocked / invalid / incompatible / host-writing candidate evidence 会影响 readiness `summary.result`。
- Readiness report generator 仍不调用 `generate-host-bridge-candidate-package`，不创建、不重写、不接受、不 apply candidate evidence。
- `HostBridgeCandidateGenerationDomain` 明确忽略无关 optional `reports/readiness-report.json` 破损，candidate 语义输入仍限于 `usage/usage.json`、`host/host-schema-capabilities.json`、`host/host-integration-audit.json`。
- `HostIntegrationReadinessReportSmoke.js` 已扩展 existing candidate summary 覆盖：在 package 副本上显式运行 candidate generator，然后验证 readiness report 只汇总且不改写 candidate。
- 新增 internal tests：`host integration package readiness report summarizes existing candidate` 与 `host bridge candidate generation ignores invalid optional readiness report`。
...
- 下一轮进入 Round 2：shared Tooling candidate domain / package reader integration。

### 2026-06-22 Host Bridge Candidate Generator First Slice Goal Guide 快照

用户已批准 `Host Bridge Candidate Generator First Slice` 作为 Static Artifact POC Partner Handoff Kit 之后的下一阶段。
- 执行指南见 [Host Bridge Candidate Generator First Slice Goal 模式执行指南](host-bridge-candidate-generator-goal-mode-execution-guide.md)。
- 执行预算为 6 轮：Round 1 baseline / command contract / source-of-truth audit，Round 2 shared Tooling candidate domain / package reader integration，Round 3 CLI command / diagnostics / output guard，Round 4 smoke fixture / determinism / docs hardening，Round 5 buffer / edge-case hardening / compatibility closure，Round 6 final validation / PASS-FAIL closure。
- 本阶段定位是 standalone package candidate generator first slice：从既有 Host Integration Package artifact 生成 review-only Host Bridge Candidate evidence。
- `export-host-integration-package-project` 默认仍不得自动生成 `host/host-bridge-candidate.json`；readiness report 可以读取已有 candidate summary，但不得代替 generator 生成 candidate。
- 本阶段不写 confirmed Host Bridge，不做 generated apply，不做 POC-2 catalog projection，不接 Runtime Preview Bridge / Sinan Runtime / Unity / Host SDK / 完整 host save，不做 Rollback / Trace Replay / Flashback / Presentation IR / Host Schema action policy expansion。
...
用户已批准近期计划的第一阶段：`Host Integration Package CLI`。本阶段目标不是再写契约，而是把 Host Integration Partner Readiness 阶段确认的静态 artifact 收成真实 CLI package 导出命令。
- 执行指南见 [Host Integration Package CLI Goal 模式执行指南](host-integration-package-cli-goal-mode-execution-guide.md)。
- 预算为 6 轮会话：Round 1 baseline / command contract，Round 2 package domain / manifest writer，Round 3 existing artifact assembly，Round 4 source copy / source-map / anchor-map / minimal report，Round 5 smoke / determinism / docs，Round 6 final validation / PASS-FAIL closure。
- 目标命令为 `export-host-integration-package-project <workspace> -o <out-dir>`，输出 `manifest.json`、`source/`、`graph/project-ir.json`、`usage/usage.json`、`host/host-schema-capabilities.json`、`host/host-integration-audit.json`、`localization/l10n.csv`、`localization/anchor-map.json`、`source-map/source-locations.json` 与 `reports/readiness-report.json`。
- 本阶段允许做最小 package-level readiness report，但不做完整 readiness report generator 产品化；下一阶段若继续，应重新批准。
- 本阶段继续禁止 Host Bridge candidate generator、generated apply、Sinan Runtime Integration、Runtime Preview Bridge、Unity / Host SDK、完整 host save、Rollback / Trace Replay / Flashback、Presentation IR、Host Schema action policy 扩张或任何 Sinan / Unity / Bird hard dependency 进入 `src/Internal`。
- 执行者每轮必须 Debug 自检、架构自检、验证通过后提交推送，再进入下一轮。
...
Host Integration Partner Readiness 已完成 Round 4 Host Bridge Candidate Contract + Static Artifact Fixtures。
- 新增 [Host Bridge Candidate Contract](host-bridge-candidate-contract.md)，定义 `host/host-bridge-candidate.json` 的 candidate kind、status、confidence、conflict、manual review、generated ownership、Sinan catalog projection boundary 和 `writesHostData = false` POC-1 边界。
- 新增 [Host Integration Partner Readiness Fixtures](host-integration-partner-readiness-fixtures.md) 与静态 fixture pack [fixtures.json](host-integration-static-fixtures/fixtures.json)，覆盖 minimal dialogue、branching、localization、missing speaker、unknown action、unsupported feature 和 source diagnostic 七类场景。
- [Host Integration Package Contract](host-integration-package-contract.md) 已接入 Round 4 契约，`host/host-bridge-candidate.json` 从 planned 变为 Round 4 contract；static fixture pack 只作为 docs/test evidence，不是 Compiler sample、Runtime sample 或宿主输出。
- 本轮只做 docs / static fixture 收口；未实现 Host Bridge candidate generator、generated apply、Host SDK、Sinan Runtime、Runtime Preview Bridge、完整 host save、Rollback / Trace Replay / Flashback、Presentation IR 或 Host Schema action policy 扩张。
- 当前缺口仍包括 static artifact smoke / readiness report shape、Sinan Static Artifact POC planning note、POC-1 checklist 和 final validation report。
- 下一轮进入 Round 5：Static Artifact Smoke / POC Planning / Buffer；继续禁止 Sinan Runtime Integration、Runtime Preview Bridge、hard dependency、Sinan-specific core semantics、完整 host save、Rollback / Trace Replay / Flashback、Presentation IR 或 Host Schema action policy 扩张。

### 2026-06-21 Host Integration Partner Readiness Round 3 快照

Host Integration Partner Readiness 已完成 Round 3 Source Location + Localization Anchor Export Contract。
- 新增 [Source Location External Contract](source-location-external-contract.md)，定义 Integration Package / report / diagnostics 可依赖的 source ref、package path、source-map artifact、location role、precision / availability 与 importer 规则。
- 新增 [Localization Anchor Export Contract](localization-anchor-export-contract.md)，定义 `localization/l10n.csv`、`localization/anchor-map.json`、anchor 语义、line identity / graph ref 连接、translation alignment 状态与 partner refs 边界。
- [Host Integration Package Contract](host-integration-package-contract.md) 已接入 Round 3 契约，`source-map/source-locations.json` 与 `localization/anchor-map.json` 从 planned 变为 Round 3 contract。
- 本轮只做 docs / external contract 收口；未实现 package command、Host Bridge candidate generator、Sinan Runtime、Runtime Preview Bridge、完整 host save、Rollback / Trace Replay / Flashback、Presentation IR 或 Host Schema action policy 扩张。
- 当前缺口仍包括 Host Bridge candidate contract、static fixture set、Sinan Static Artifact POC planning note、POC-1 checklist、readiness report shape 和 final validation report。
- 下一轮进入 Round 4：Host Bridge Candidate Contract + Static Artifact Fixtures；继续禁止 Sinan Runtime Integration、Runtime Preview Bridge、hard dependency、Sinan-specific core semantics、完整 host save、Rollback / Trace Replay / Flashback、Presentation IR 或 Host Schema action policy 扩张。
...
P5 SelfHostedEditor Runtime authoring / productization 已完成第七轮 Runtime status surface，不代表 P5 已完成。
- 本轮审计见 [SelfHostedEditor P5 Runtime Status Audit](self-hosted-editor-p5-runtime-status-audit.md)。
- 新增 `RuntimeStatusSurfaceModelBuilder` 与 `RuntimeStatusPanelController`，sidebar Runtime meta 现在显示 provider、current node、visible choices、visible step count、pending action、Runtime error 与 query provider 来源。
- Runtime compact payload 现在附带脱敏 `queryProvider` summary，只保留 `mock` / `recorded` / `internal` / `delegate-unavailable` 来源和 mock / recorded count，不暴露 mock value、argument value 或 host payload。
...
- 后续已由 P5 Round 7 接上 Runtime status surface；该轮展示 current node、visible choices、visible step count、provider、pending action、Runtime error 与 query provider 来源，并继续禁止重算 Runtime state、复制 Runtime evaluator / dispatcher 或扩大 Host action policy。
...
P3 Round 11 已完成 Runtime State 最小模型与 `ValidateStateAgainstCurrentScript` shape，不宣称 P3 完成。

- 实现审计见 [SelfHostedEditor P3 Runtime State Audit](self-hosted-editor-p3-runtime-state-audit.md)。
- `NarrativeRuntimeExportStateModel` 定义正式 `inscape.runtime-state` 最小 shape：`format`、`formatVersion`、`runtimeVersion`、`scriptVersion`、`position`、`flow`、`facts`、`random`、`host.checkpointId`。
- `NarrativeRuntime.ExportState(scriptVersion, hostCheckpointId)` 从当前 Runtime 投影正式 Runtime State；`host.checkpointId` 只作为 opaque id 保存，不由 Inscape 解释。
- `NarrativeRuntime.ValidateStateAgainstCurrentScript(state, currentScriptVersion)` 输出 `inscape.runtime-state-validation`，支持 `compatible` / `migratable` / `incompatible` 三档结果和结构化 diagnostics。
...
- P3 Round 11 已在后续快照完成 Runtime State 最小模型；下一步进入端到端 smoke 与文档收口。
