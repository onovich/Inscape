...
- 宿主 Schema 草案：新增 `hostSchema` 项目配置字段与 `export-host-schema-template` CLI 命令，用于生成 `inscape.host-schema` JSON 模板，先描述纯查询和宿主事件清单，不改变当前 DSL 解析或 Bird 导出行为。详见 [宿主 Schema 草案](host-schema.md)。
...
5. 宿主 Schema 接入 VSCode：
   - `export-host-schema-template` 已能生成查询 / 事件清单模板。
   - 下一步可以让 VSCode 读取 `hostSchema`，为未来查询名、事件名和参数名提供补全 / Hover，但暂不设计正式条件语法。
