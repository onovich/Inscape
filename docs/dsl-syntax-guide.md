...
这很接近“给宿主一个标签/绑定键”的直觉，比如：

- `[bg: courtroom]`
- `[timeline: court_intro]`

所以 `[]` 当前确实比 `@scene` 这类 metadata 更容易形成稳定心智模型。
...
- `[]` 主要是宿主标签 / 绑定提示，不一定是事件。
   - 它也可以是背景、资源、别名。
- `@` 主要是 metadata，不只标记 block。
   - 它也可以表达入口，或者表达带 phase 的 hook 时机。
