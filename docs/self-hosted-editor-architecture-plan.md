...
- 2026-06-17 当前状态：P1 post-40 已补固定 Electron IPC command boundary，并继续落地真实 Electron workspace open / read / write buffer、recovery snapshot IO 与 autosave/flush execution helper 的局部闭环。preload 内部通过 `inscape.self-hosted-editor.backend.invoke` 转发白名单 editor command，main process 复用 payload validator，持有 `ElectronWorkspaceSessionStore`，可打开目录、扫描 `.inscape`、读入 `DocumentBufferStore`、执行 `document-buffer.save` / `save-all` 真实磁盘写回，并在 dirty edit 后写入 `.inscape-workspace/recovery/` snapshot、打开 workspace 时扫描 text-free recovery status、保存成功后清理 snapshot；`runAutosave()` / `flushDirtyDocuments()` 可执行 plan 并通过真实 save 路径写盘；renderer 仍不知道 IPC channel，也不能直接访问 Node / fs / shell。真实 idle timer、Electron close/switch/app-exit lifecycle 挂接、restore-discard-later 和 GUI edit-save-recovery smoke 仍待后续。
