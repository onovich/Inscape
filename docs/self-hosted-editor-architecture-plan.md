...
- 2026-06-17 当前状态：P1 post-40 已补固定 Electron IPC command boundary，并继续落地真实 Electron workspace open / read / write buffer 的局部闭环。preload 内部通过 `inscape.self-hosted-editor.backend.invoke` 转发白名单 editor command，main process 复用 payload validator，持有 `ElectronWorkspaceSessionStore`，可打开目录、扫描 `.inscape`、读入 `DocumentBufferStore`、执行 `document-buffer.save` / `save-all` 真实磁盘写回，并返回 `embedded-desktop` ProjectSession 摘要；renderer 仍不知道 IPC channel，也不能直接访问 Node / fs / shell。autosave flush、recovery snapshot 写入 / restore-discard-later 和 GUI edit-save-recovery smoke 仍待后续。
