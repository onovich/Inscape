...
- 已新增本地化会话草稿：本地化视图支持在当前会话内填写译文草稿，并用 `empty` / `draft` 状态区分；当前既可下载浏览器 draft CSV，也可选择真实旧 CSV，经开发宿主 `/api/localization-update` 复用 CLI `update-l10n-project` 导出真实 updated CSV。开发宿主会按 `sessionId` 记住本次选择的 previous CSV，后续 review / update 可复用这份旧表而不反复上传整份 CSV；这份会话 baseline 现在受 dev-host session cache 的 2 小时 idle TTL 和 64 条容量上限保护，并可通过 `/api/session-cache-status` 观察大小与淘汰计数。若 previous CSV 缺少 `anchor` 与 `translation` header，共享 Tooling 入口会拒绝该输入，避免 host config CSV 被误当作 localization CSV。若浏览器支持 native file handle，打开旧 CSV 后还可把 updated CSV 直接写回原文件；前端只传 session、必要时传 `previousCsv`、以及 anchor-based translation overrides，不自己拼真实 CSV 语义，也不自己重造文件写回语义。
...
- 已新增 Stable Node Map 显式审查入口：顶栏 `Node Map` 会通过开发宿主 `/api/node-map-review` 运行共享 CLI `update-node-map-project --report`，展示 shared report 的 `new / renamed / manual-review / conflict / missing` 摘要与审查项，支持 source jump，并允许下载生成的 `inscape.node-map.json`。candidate apply 语义已收成 Tooling / CLI 共享契约；SelfHostedEditor UI 会先 dry-run preview，再要求 `Confirm Apply`。dev-host HTTP 路径保持 download-ready 语义；Electron desktop 路径会先走 `workspace.write-back-backup`，再通过 desktop-only `stable-node-map.write-sidecar` 写回 node-map sidecar。
...
- 2026-06-02 / 2026-06-17 当前状态：SelfHostedEditor L10N 视图已补上“真实旧 CSV 选择 + 真实 updated CSV 导出 / 直接替换”，并继续把 linked baseline 的宿主状态说清楚。共享层 / CLI 新增 `--translation-overrides`，允许在 `update-l10n` / `update-l10n-project` 前按 anchor 吃掉前端草稿覆盖；开发宿主 `/api/localization-review` 与 `/api/localization-update` 现在按 `sessionId` 记住 previous CSV baseline，前端只在旧 CSV 新增或变化时重传，后续可只传 session 与 translation overrides，由 CLI 继续产出真实 updated CSV。previous CSV 读取入口现在必须看到 `anchor` 与 `translation` header，误传 host config CSV 会在 `Internal/Tooling` 失败而不是生成伪 updated CSV；dev-host compact update payload 也会声明 `generatedBy: "update-l10n-project"`、`writesWorkspaceFile: false`、backup `not-written-by-dev-host` 与恢复提示。`LocalizationEditorController` 现在会显示 review baseline、读取真实旧 CSV、保留 session draft overrides、按状态筛选当前可见 review 行、显示当前 override / 可见 draft / 导出 readiness 状态，并在浏览器支持 native file handle 时把 updated CSV 直接写回已链接旧文件；linked baseline 会根据当前 anchor 草稿切换 `linked clean` / `linked N unsaved`，`Replace previous CSV` 也只在确实有未保存 linked 草稿时可用；不支持 native file handle 时仍保留浏览器下载导出。该链路已补 `check:model`、`check:localization-update` 与 `check:localization-update-http` 回归，保持“共享语义留在 Internal / CLI，消费端只做薄适配”。
- 2026-06-17 当前状态：P2 workbench integration smoke 已补上跨能力 HTTP gate。`check:workbench-integration-http` 会在同一个 dev-host server 里串起 localization review / update、line-map refresh、stable node map review / apply，并覆盖 hosted empty、missing-baseline error、session baseline success、node-map dry-run/apply、backup metadata、recovery hint 与 non-content session cache status；该 smoke 只验证工作台可消费 payload，不新增 host 端 scoring、migration 或 apply 语义。
- 2026-06-17 当前状态：P2 Round 13 文档与 ADR 收口已完成，审计见 [SelfHostedEditor P2 Documentation And ADR Closure Audit](self-hosted-editor-p2-doc-adr-closure-audit.md)。本轮不新增 ADR，P2 仍落在 ADR 0013 / 0017 / 0018 / 0019 / 0020 的既有决策内；stable node id、localization diff alignment 与 l10n extraction contract 已同步 P2 口径。Host Schema / Host Bridge / Unity-Bird 集成只允许在 Round 14 完整验证通过并记录 `P2 PASS` 后进入。
- 2026-06-17 当前状态：P2 Round 14 全量验证已通过，最终报告见 [SelfHostedEditor P2 Final Validation Report](self-hosted-editor-p2-final-validation-report.md)。P2 stable identity / localization review 判定为 PASS，Post-P2 host integration work allowed: YES。后续 P2.5 可以启动，但仍必须保持 Host Schema / Host Bridge / Unity-Bird 只消费共享 Tooling / LanguageServer / adapter contract，不把宿主资源映射或 Unity 依赖放入 `Internal`。
...
- 2026-06-16 当前状态：P1 Round 25 已把 `DocumentBufferStore` 手动 Save 命令面接入 contract layer。`saveDocument` / `saveAll` 先通过 backend workspace boundary、write target catalog 与 `baseRevision` guard，返回 text-free saved / error status，并通过 `EditorBackendClient.documentBuffer.*`、preload whitelist 和 fake embedded transport 暴露为受控 editor command；真实 Electron 文件 IO、autosave debounce、flush 和 recovery 仍在后续 P1 轮次。
- 2026-06-16 当前状态：P1 Round 26 已补 `lastSavedRevision` clean baseline 与 `disk-conflict` save error contract。edit 推进 dirty / revision 时不覆盖 saved baseline，save 成功才刷新 baseline；磁盘 hash 冲突只通过 text-free save error 暴露，仍不让 renderer 获得文件系统写能力。
- 2026-06-16 当前状态：P1 Round 27 已补 backend autosave plan contract。autosave 只在 idle debounce ready 后为 dirty `.inscape` 当前 revision 生成 save request，旧 pending revision 会被标为 `stale-autosave-revision`；本轮仍不启动真实 timer 或写盘。
- 2026-06-16 当前状态：P1 Round 28 已补 flush lifecycle guard contract。`buildFlushPlan()` 覆盖 manual Save / close window / switch workspace / app exit，使用当前 latest revision 生成 text-free flush request；unsafe target 与 save failure 会进入 UI-visible blocking / failure state，仍不执行真实文件 IO 或 recovery snapshot。
- 2026-06-16 当前状态：P1 Round 29 已补 recovery snapshot write payload contract。`buildRecoverySnapshotPlan()` 可从 dirty backend buffer 生成含文本的 recovery snapshot payload，并把 text-free recovery status / cleanup request 分离出来；真实文件 IO、启动扫描和恢复 UI 仍在后续轮次。
- 2026-06-16 当前状态：P1 Round 30 已补 recovery UI/status/action contract。ProjectSession `recoveryStatus` 会投影为 session panel 的 recovery label、可恢复文件列表和 restore / discard / later action request；UI 只消费 text-free summary，不读取 snapshot 正文或文件系统。
- 2026-06-16 当前状态：P1 Round 31 已补 `.inscape-workspace/` internal directory plan contract。open workspace 可得到 recovery / backups / cache 的发现/创建计划、non-project-truth 标记、cache 可重建语义与默认 `.gitignore` 建议；真实 mkdir / gitignore 写入仍待 IO 层。
- 2026-06-17 当前状态：P1 Round 32 / post-40 E 已补 write-back backup plan 与真实 Electron IO。localization CSV、node-map sidecar、line-map sidecar 写回前会生成 `.inscape-workspace/backups/` 下的 text-free backup request；desktop-only `workspace.write-back-backup` command 已接到 Electron main process，按同一 plan 复制三类 sidecar、执行 count-and-age retention cleanup，并保持响应不泄露 CSV / sidecar 内容。
- 2026-06-17 当前状态：P1 Round 33 已补 assets import policy contract，P1 post-40 F 已补真实 Electron assets import IO。外部图片、音频、CSV 导入会生成复制到 workspace `assets/images|audio|data` 的 text-free plan，不持久化 workspace 外路径；`assets/**` 写目标优先于扩展名规则，Electron main process 通过 desktop-only `workspace.import-assets` 选择外部文件并执行复制。
- 2026-06-17 当前状态：P1 Round 34 已补 settings schema contract。`EditorBackendSettingsSchemaModel` 集中默认值并区分 global user preferences 与 workspace project behavior；设置页和真实 settings persistence 仍待后续 backend command / IO 层。
- 2026-06-17 当前状态：P1 Round 35 已补 desktop v0 contract smoke。`smoke:desktop` 串起 open workspace、DocumentBuffer edit、autosave、manual Save、recovery、authoring bridge 与 Runtime choose action；不启动 Electron、不执行真实文件 IO，Windows package smoke 留给下一轮。
- 2026-06-17 当前状态：P1 Round 36 已补等价本机启动 smoke。`smoke:desktop-startup` 验证 package / Desktop entry / Workbench / preload / contract loop；当轮尚未安装 Electron runtime、尚未生成 Windows package。
- 2026-06-17 当前状态：P1 Round 37 已补真实 Electron runtime / 启动入口 smoke。`smoke:desktop-runtime` 通过 Electron CLI `--version` 与受保护 runtime probe 验证 `Desktop/ElectronMain.js` 可在真实 Electron main process 中加载，并检查 BrowserWindow 安全默认；仍未生成 Windows package、不打开 GUI、不做真实 workspace 文件 IO。
- 2026-06-17 当前状态：P1 Round 38 已补 Windows package script / electron-builder config contract。`check:desktop-package` 验证 `main`、`package:windows`、build files 白名单、Windows `dir` x64 target 与 artifact readiness；package script 已可用，但 artifact 未生成时仍记录 `windows-package-not-generated`。
- 2026-06-17 当前状态：P1 Round 39 已补 Windows package artifact smoke。`package:windows` 可生成 `dist/win-unpacked/Inscape SelfHostedEditor.exe` 与 `resources/app.asar`，`smoke:desktop-package` 验证 artifact 存在、体积和 builder metadata；仍未执行 GUI 打开 workspace / 编辑保存 / recovery 提示 smoke。
- 2026-06-17 当前状态：P1 Round 40 已补 packaged app protocol / asset loading guard。Electron main 使用 `inscape-self-hosted-editor://app/` 加载 Workbench，并以路径白名单服务 `Resources/`、`Scripts/`、Monaco 与 packaged samples；renderer 仍不获得 Node/fs/shell/IPC，真实 GUI workspace/edit-save/recovery smoke 仍待后续。
- 2026-06-17 当前状态：P1 post-40 已补固定 Electron IPC command boundary，并继续落地真实 Electron workspace open / read / write buffer、recovery snapshot IO、autosave/flush execution helper、main-process lifecycle 挂接、recovery action IO、write-back backup IO、assets import IO、GUI recovery smoke、GUI Preview smoke 与 Windows packaged GUI smoke。preload 内部通过 `inscape.self-hosted-editor.backend.invoke` 转发白名单 editor command，main process 复用 payload validator，并通过 `ElectronWorkspaceLifecycle` 与 IPC 共享同一个 `ElectronWorkspaceSessionStore`；该 store 可打开目录、扫描 `.inscape`、读入 `DocumentBufferStore`、执行 `document-buffer.save` / `save-all` 真实磁盘写回，并在 dirty edit 后写入 `.inscape-workspace/recovery/` snapshot、打开 workspace 时扫描 text-free recovery status、保存成功后清理 snapshot。idle autosave timer、BrowserWindow close、workspace switch 与 app-exit flush 已接到真实 save 路径；desktop-only `recovery.restore` / `recovery.discard` / `recovery.later` 已接到真实 snapshot 写回/删除/稍后处理；desktop-only `workspace.write-back-backup` 已按 shared backup plan 复制 localization CSV、node-map 和 line-map sidecar，并清理过期 backup artifact；desktop-only `workspace.import-assets` 已按 shared asset import plan 复制图片、音频、CSV 到 workspace `assets/images|audio|data`，且 renderer payload 不携带外部路径。language-session commands 已通过 `EditorBackendWorkspaceSnapshotModel` / `EditorBackendLanguageSessionRequestModel` 从 main-process 当前 `DocumentBufferStore` 构建请求；真实 BrowserWindow smoke 与 packaged exe smoke 已验证 app protocol Workbench、sandbox-compatible `ElectronPreload.cjs`、renderer preload API、manual Save、autosave / restore 路径、recovery actions、diagnostics / completions 使用 restore 后当前 buffer，以及 Preview 渲染默认样例、choice click 推进到目标 block 并 reveal editor source line。
...
P2 Round 14 的全量验证已覆盖 VSCode semantic parity、SelfHostedEditor localization review/update、line-map、node-map、workbench integration 与 semantic parity HTTP smoke。P2.5 Host Schema / Host Bridge / Unity-Bird 工作可以开启；失败缓冲轮未被消耗。
