...
- 2026-06-17 当前状态：P1 post-40 已补固定 Electron IPC command boundary，并继续落地真实 Electron workspace open / read / write buffer、recovery snapshot IO、autosave/flush execution helper、main-process lifecycle 挂接、recovery action IO 与 GUI recovery smoke。preload 内部通过 `inscape.self-hosted-editor.backend.invoke` 转发白名单 editor command，main process 复用 payload validator，并通过 `ElectronWorkspaceLifecycle` 与 IPC 共享同一个 `ElectronWorkspaceSessionStore`；该 store 可打开目录、扫描 `.inscape`、读入 `DocumentBufferStore`、执行 `document-buffer.save` / `save-all` 真实磁盘写回，并在 dirty edit 后写入 `.inscape-workspace/recovery/` snapshot、打开 workspace 时扫描 text-free recovery status、保存成功后清理 snapshot。idle autosave timer、BrowserWindow close、workspace switch 与 app-exit flush 已接到真实 save 路径；desktop-only `recovery.restore` / `recovery.discard` / `recovery.later` 已接到真实 snapshot 写回/删除/稍后处理。language-session commands 已通过 `EditorBackendWorkspaceSnapshotModel` / `EditorBackendLanguageSessionRequestModel` 从 main-process 当前 `DocumentBufferStore` 构建请求；真实 BrowserWindow smoke 已验证 app protocol Workbench、sandbox-compatible `ElectronPreload.cjs`、renderer preload API、manual Save、autosave、recovery actions，以及 diagnostics / completions 使用 restore 后当前 buffer。
