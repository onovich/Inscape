...
- 2026-06-17 当前状态：P1 post-40 已补固定 Electron IPC command boundary。preload 内部通过 `inscape.self-hosted-editor.backend.invoke` 转发白名单 editor command，main process 复用 payload validator 并只接线 `project-session.status` 的 `embedded-desktop` 摘要；renderer 仍不知道 IPC channel，真实 workspace open / file IO / save-recovery 仍待后续。
