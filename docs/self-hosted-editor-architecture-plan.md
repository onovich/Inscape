...
- 2026-05-26 未提交草案：SelfHostedEditor L10N 视图开始接入真实 Tooling presenter。开发宿主新增 `/api/localization-review`，在临时 workspace 中运行 `update-node-map-project`、`extract-l10n-project` 和 `audit-l10n-alignment-project --from <csv>`，返回 `LocalizationAlignmentReportModel.Presenter`；前端新增 `SelfHostedEditorLocalizationReviewBridge`，`LocalizationEditorController` 优先渲染 presenter items，并保留 session draft fallback。该草案还把 dev server POST JSON 统一改为 `parseJsonRequestBody()`，剥离 UTF-8 BOM，避免 PowerShell 生成的 BOM JSON 触发 500。已通过 SelfHostedEditor syntax / structure / model 检查；最小脚本 `/api/localization-review` HTTP smoke 已通过，完整 `samples/court-loop.inscape` smoke 仍会超时，需优先定位 full sample 路径。
