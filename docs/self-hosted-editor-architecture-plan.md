...
- 本地化编辑视图：当前已有从脚本临时提取的草表、会话内译文草稿和浏览器 draft CSV 导出；后续接入真实本地化 CSV、alignment review、筛选、审校与草稿写回。
...
- 2026-05-26 当前状态：SelfHostedEditor L10N 视图已开始接入真实 Tooling presenter。开发宿主新增 `/api/localization-review`，在临时 workspace 中运行 `update-node-map-project`、`extract-l10n-project` 和 `audit-l10n-alignment-project --from <csv>`；返回值已收成更窄的 UI 契约，但仍保持通用层已有的 `presenter.items` 结构，只裁掉浏览器暂时不需要的完整 audit report 和冗余字段。前端通过 `SelfHostedEditorLocalizationReviewBridge` 与 `LocalizationEditorController` 优先渲染这些 items，并保留 session draft fallback。该路径也把 dev server POST JSON 统一改为 `parseJsonRequestBody()`，剥离 UTF-8 BOM，避免 PowerShell 生成的 BOM JSON 触发 500。已通过 SelfHostedEditor syntax / structure / model 检查；后续排查显示完整 `samples/court-loop.inscape` 的底层 CLI 链路约 10 秒，因此下一步更应定位 HTTP dev-host 与响应传输，而不是 Tooling 算法本身。
...
- 仍需替换：`ScriptDocumentModelBuilder` 继续是 UI-only 草模，但 Graph 与 Preview 的正常服务路径已开始消费 Compiler project graph；诊断 marker 仍主要贴活动文件；L10N 视图正在从 session draft 过渡到 Tooling presenter，但真实 CSV 文件选择 / 写回仍未完成；Preview 仍不是 Runtime Player；Graph 位置仍是 session memory。后续每次迭代应尽量挑一个窄消费者替换为 Internal 输出，而不是在前端草模上继续叠语义。
