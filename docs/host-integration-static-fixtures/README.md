...
The static fixture smoke does not compile source. The package CLI smoke invokes only the Inscape CLI package export command against a temporary local workspace. The readiness report smoke invokes only package export and the standalone package readiness report generator against temporary local files. The Host Bridge Candidate generator smoke invokes package export and the standalone candidate generator, but keeps the output as review-only evidence. These smokes do not run Runtime, connect Unity / Host SDK / Sinan Runtime, write host data, or generate confirmed Host Bridge mappings.
